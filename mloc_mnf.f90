!> Procedures for reading and writing files in MLOC Native Format (MNF)
...
!    character(len=5) :: data_author
   ...
   do
      ...
      ! Missing STOP record termination
      if (linein(1:3) .eq. 'EOF') then
         write (msg,'(a,i3)') 'read_mnf_134: missing STOP record for event ', iev
         call warnings (trim(msg))
         exit
      end if
      ...
      if (linein(1:1) .eq. 'E')  then
         ...
         if (annotation(iev) .eq. ' ') annotation(iev) = event_comment(iev)(1:20)
         if (debug) write (io_log,'(a,i3,2a)') 'read_mnf_134: for event ', iev, ' annotation = ', trim(annotation(iev))
      end if
      ...
      if (linein(1:1) .eq. 'I') then
         ...
         if (debug) then
            write (io_log,'(a,i3,2a)') 'read_mnf_134: for event ', iev, ' evid_source = ', evid_source(iev)
            write (io_log,'(a,i3,2a)') 'read_mnf_134: for event ', iev, ' evid = ', evid(iev)
         end if
      end if
      ...
      if (linein(1:1) .eq. 'H') then
         if (linein(3:3) .eq. '=' .or. .not.h_set) then
            ...
            if (debug) then
               write (io_log,'(a,i3,a,i4)') 'read_mnf_134: for event ', iev, ' iyre = ', iyre(iev)
               write (io_log,'(a,i3,a,i2)') 'read_mnf_134: for event ', iev, ' mone = ', mone(iev)
               write (io_log,'(a,i3,a,i2)') 'read_mnf_134: for event ', iev, ' idye = ', idye(iev)
               write (io_log,'(a,i3,a,i2)') 'read_mnf_134: for event ', iev, ' hour_inp = ', hour_inp(iev)
               write (io_log,'(a,i3,a,i2)') 'read_mnf_134: for event ', iev, ' min_inp = ', min_inp(iev)
               write (io_log,'(a,i3,a,f5.2)') 'read_mnf_134: for event ', iev, ' sec_inp = ', sec_inp(iev)
               write (io_log,'(a,i3,a,f8.4)') 'read_mnf_134: for event ', iev, ' lat_inp = ', lat_inp(iev)
               write (io_log,'(a,i3,a,f9.4)') 'read_mnf_134: for event ', iev, ' lon_inp = ', lon_inp(iev)
               write (io_log,'(a,i3,a,f8.2)') 'read_mnf_134: for event ', iev, ' time_inp = ', time_inp(iev)
            end if
            ...
            if (debug) write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' depth_inp(iev,1) = ', depth_inp(iev,1)
            ...
            if (debug) write (io_log,'(a,i3,2a)') 'read_mnf_134: for event ', iev, ' depth_inp_c = ', depth_inp_c(iev)
            ...
            if (debug) then
               write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' depth_inp(iev,2) = ', depth_inp(iev,2)
               write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' depth_inp(iev,3) = ', depth_inp(iev,3)
            end if
            ...
            if (debug) write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' sece_sigma = ', sece_sigma
            ...
            if (debug) then
               write (io_log,'(a,i3,a,i3)') 'read_mnf_134: for event ', iev, ' alpha_smin_in = ', alpha_smin_in
               write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' smin_in = ', smin_in
               write (io_log,'(a,i3,a,f5.1)') 'read_mnf_134: for event ', iev, ' smaj_in = ', smaj_in
            end if
            
            ! Calibration code
            cal_code_input(iev) = linein(90:93)
            if (debug) write (io_log,'(a,i3,2a)') 'read_mnf_134: for event ', iev,&
             ' cal_code_input(iev) = ', cal_code_input(iev)
            ...
            if (debug) write (io_log,'(a,i3,a)') 'read_mnf_134: for event ', iev, ', return from hdf_match'
            
            ! Set starting hypocentral parameters.
            call starting_hypocenter (iev)
            if (debug) write (io_log,'(a,i3,a)') 'read_mnf_134: for event ', iev, ', return from starting hypocenter'
            
            ! Initialize tau-p software for calculating travel times
            call depset (depthp(iev,0), usrc)
            if (debug) write (io_log,'(a,i3,a)') 'read_mnf_134: for event ', iev, 'return from depset'
         end if
      end if
      ...
      if (linein(1:1) .eq. 'M') then
         if (linein(3:3) .eq. '=' .or. .not.m_set) then
            ...
            m_set = .true.
            ! Auto-plot stars based on magnitude
            if (star_auto) then
               do i = 1,n_star_table
                  if (rmag(iev) .ge. star_table(i,1) .and. rmag(iev) .le. star_table(i,2)) then
                     n_star = n_star + 1
                     iev_star(n_star) = iev
                     star_size(n_star) = star_table(i,3)
                  end if
               end do
            end if
         end if
      ...
      if (linein(1:1) .eq. 'P') then
         if (.not.phase_ok(linein(24:31))) then  ! Check if this is a junk phase
            ...
            if (plogout) write (io_plog,'(a)') trim(msg)
            cycle
         end if
         ird = ird + 1
         if (debug) write (io_log,'(a,i3,a,i4)') 'Phase reading record for event', iev, ' ird = ', ird
         mnf_line(iev,ird) = line_number
         diff_line(iev,ird) = 0
         if (ird .gt. n_arrtim_max) call oops ('read_mnf_134: ird exceeds n_arrtim_max')
         nst(iev) = ird
...
!             data_author = linein(103:107) ! not used
         ...
         if (nsmd) then
            if (linein(81:82) .ne. '  ') then ! Don't bother checking if there is no network/deployment code in the phase record
               ...
               call write_nsmd (io_nsmd, dslc_test, jdate)               
            end if
         end if
         ...
         if (kcode(iev,ird) .eq. -1 .and. ifsm) then ! kcode = -1 for stations not found in the current station list
            if (linein(81:82) .ne. '  ') then ! Don't bother checking if there is no network/deployment code in the phase record
               ...
               call juldat (ipay(iev,ird), ipamo(iev,ird), ipad(iev,ird), julday, 0)
               jdate = ipay(iev,ird)*1000 + julday
               call write_ifsm (io_ifsm, ds_test, jdate)
            end if
         end if
         ...
         if (n_skip .gt. 0 .and. fcode(iev,ird) .eq. ' ') then
            do i = 1,n_skip
   ...
   write (msg,'(a,i6,a)') 'read_mnf_134: ', nphreid, ' phases re-identified for iteration 0'
...
!             end if
      ...
      if (linein(1:1) .eq. 'P') then
         ...
         mnf_line(iev,ird) = line_number
         diff_line(iev,ird) = 0
         if (ird .eq. n_arrtim_max) call oops ('read_mnf_13: ird exceeds n_arrtim_max')
         nst(iev) = ird
         fcode(iev,ird) = linein(3:3)
         stname(iev,ird) = linein(5:9)
         dist_az(iev,ird) = linein(12:21)
...
! and copied forward. If they were in an older format they are translated to v1.3 as far as possible.
   ...
   integer :: io_unit, iev, ird, it, it1, calibration_type, i, indx(n_arrtim_max)
!   integer :: scale_length
   real :: dts, dot, alpha, xl1, xl2, ddep, lat_out, lon_out, min_depth_uncertainty,&
    depth_out, deltiev(n_arrtim_max)
...
! 11/16/2017: v1.4.2, adding deployment/network code to station code for each phase reading
   ...
   character(len=1) :: depth_code
!    character(len=1) :: usage
   character(len=1) :: c1
   character(len=1) :: fcode_pr
   integer :: io_unit, iev, ird, it, it1, calibration_type
!   integer :: scale_length
   real :: dts, dot, alpha, xl1, xl2, ddep, lon_test, min_depth_uncertainty
...
!   logical :: skipp
   ...
   do
      ...
      if (linein(1:1) .eq. 'D') then ! Differential time record
         ...
         ndiff = ndiff + 1
         if (ndiff .gt. n_diff_max) then
            write (msg,'(a,i4,a)') 'read_mnf_15: ndiff exceeds maximum (', n_diff_max, '). Remaining data skipped'
            call warnings (trim(msg))
            ndiff = ndiff - 1
            exit
         end if
         ...
         call trtm (delt(iev_template,ird_template), nphase, tt, dtdd, dtdh, dddp, phcd)
...
! Match an event to a list read from an HDF file
   ...
   if (found_hdf) then
      ...
      if (debug) write (io_log,'(a,i3,1x,2f10.4,f5.1,1x,a1,1x,2f5.1,f10.3)')&
       'hdf_match: ', iev, lat_hdf(iev), lon_hdf(iev), depth_hdf(iev,1), depth_hdf_c(iev),&
       depth_hdf(iev,2), depth_hdf(iev,3), time_hdf(iev)
...
!   logical :: depth_constraint
   ...
   if (debug) write (io_log,'(a,i3,a,f8.4)') 'starting_hypocenter: event ', iev, ' latp = ', latp(iev,0)
   ...
   if (debug) write (io_log,'(a,i3,a,f9.4)') 'starting_hypocenter: event ', iev, ' lonp = ', lonp(iev,0)
   ...
   if (depth_cf(iev,1) .ge. 0.) then ! Depth settings taken from command file
    ...
    ! taken from the input data file if depth is constrained
      ...
      depthp_plus(iev) = depth_default_plus
      depthp_minus(iev) = depth_default_minus
      if (debug) then
...
!       depset_pr(iev) = depth_inp_c(iev)
   ...
   if (debug) then
      write (io_log,'(a,i3,a,f8.4)') 'starting_hypocenter: event ', iev, ' depthp = ', depthp(iev,0)
      write (io_log,'(a,i3,2a)') 'starting_hypocenter: event ', iev, ' depset_pr = ', depset_pr(iev)
      write (io_log,'(a,i3,a,f8.4)') 'starting_hypocenter: event ', iev, ' depthp_plus = ', depthp_plus(iev)
      write (io_log,'(a,i3,a,f8.4)') 'starting_hypocenter: event ', iev, ' depthp_minus = ', depthp_minus(iev)
   end if
   ...
   if (debug) write (io_log,'(a,i3,a,f8.2)') 'starting_hypocenter: event ', iev, ' time0 = ', time0
   ...
   if (debug) then
      write (io_log,'(a,i3,a,i2)') 'starting_hypocenter: event ', iev, ' hourp = ', hourp(iev,0)
      write (io_log,'(a,i3,a,i2)') 'starting_hypocenter: event ', iev, ' minp = ', minp(iev,0)
      write (io_log,'(a,i3,a,f5.2)') 'starting_hypocenter: event ', iev, ' secp = ', secp(iev,0)
   end if
