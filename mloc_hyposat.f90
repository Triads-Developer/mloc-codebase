...
module mloc_hyposat
   ...
   integer :: isd
   integer :: jmod
   real, dimension(max_layers) :: h
   real :: rmax
   real, dimension(2,max_layers) :: v0
   real :: zmax
   real :: zoold
   character(len=4), dimension(max_layers) :: azo
!    common /model/  v0(2,max_layers), h(max_layers), rmax, zmax, jmod, azo(max_layers), imoh, icon, ipd, isd, zoold

   integer :: ib
   integer, dimension(np,3,3) :: ion
!    integer :: iql ! converted to an argument to subroutine reflex from ttloc2
   integer :: iqq
   integer :: jh1
   real :: aa
   real, dimension(3) :: del
   real, dimension(2,max_layers) :: fa
   real, dimension(2,max_layers) :: g
   real, dimension(2) :: pa
   real, dimension(2) :: pa1
   real :: pim
   real :: piref
   real, dimension(np,3,3) :: ppp
   real, dimension(2) :: rr
   real, dimension(2) :: tt_ ! Renamed from "tt" in the original code to solve a conflict with mloc_declare
   real, dimension(np,3,3) :: ttp
   real, dimension(2,max_layers) :: v
   real, dimension(2,max_layers) :: v2
   real, dimension(2) :: vhq
...
!     vhq(2), pa1(2), piref, v(2,max_layers), g(2,max_layers), v2(2,max_layers), del(3), phaseref, conv
   ...
   character(len=8), dimension(np) :: phlist
...
!
   ...
   real, dimension(mphas) :: ttc
   real, dimension(mphas) :: dtdd
   real, dimension(mphas) :: dtdh
   character(len=8), dimension(mphas) :: phcd

   character(len=1) :: czo1
   character(len=4), dimension(max_layers) :: az
...
!    integer :: k1
   ...
   integer :: m
   integer :: mult
   integer, dimension(max_layers) :: ndisc
   integer :: ni
   integer :: nphas
   integer :: nphas1
   integer :: nphas2
...
!    real :: dpdd2
   ...
   real :: tt1
   real, dimension(np) :: tti
   real :: vmax
   real :: vv
   real, dimension(max_layers) :: z
   real :: zdiff
   real, dimension(3) :: zo
                  ...
                  do klr = 1,3
                     ...
                     if (abs(rr(2)-del1) .lt. eps) then
                        ...
                        if (ion(iph,iql,klr) .eq. 1) then
                           ttp(iph,iql,klr) = tt_(2)
                           ppp(iph,iql,klr) = pa(2)
                        else
                           if (tt_(2) .lt. ttp(iph,iql,klr)) then
                              ttp(iph,iql,klr) = tt_(2)
                              ppp(iph,iql,klr) = pa(2)
                           end if
                        end if
