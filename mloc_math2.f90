...
subroutine ell2cv (kcrit, alpha, sminor, smajor, t11, t12, t22)
    ...
    ! Subroutine: ell2cv
    ! Purpose:
    !   This subroutine calculates the equivalent 2x2 covariance matrix 
    !   based on a confidence ellipse and a critical value. The LAPACK 
    !   routine `dsyev` is used to compute eigenvalues and eigenvectors 
    !   of this 2x2 covariance matrix.
    !
    ! Inputs:
    !   kcrit   - Critical value, typically related to a statistical 
    !             confidence level (e.g., chi-squared value).
    !   alpha   - Strike of the semi-minor axis, given in degrees.
    !   sminor  - Length of the semi-minor axis.
    !   smajor  - Length of the semi-major axis.
    !
    ! Outputs:
    !   t11, t12, t22 - Elements of the 2x2 covariance matrix.
    !
    !*********************************************************************
    ...
    real :: a(2,2), eigenvalues(2)
    real :: work(3)
    integer :: info

    ! Convert alpha (semi-minor axis angle) from degrees to radians
    cos_alpha = cos(alpha * (pi / 180.0))
    sin_alpha = sin(alpha * (pi / 180.0))

    ! Precompute squared terms and double-angle sine term
    cf2 = cos_alpha**2
    sf2 = sin_alpha**2
    csf = sin(2.0 * alpha * (pi / 180.0))   ! sin(2*alpha) in radians

    ! Compute the eigenvalues (lambda1 and lambda2)
    lambda1 = kcrit * smajor**2  ! Eigenvalue for the semi-major axis
    lambda2 = kcrit * sminor**2  ! Eigenvalue for the semi-minor axis

    ! Fill the 2x2 matrix with the computed values
    a(1,1) = lambda1 * cf2 + lambda2 * sf2  ! Covariance matrix element (1,1)
    a(1,2) = 0.5 * csf * (lambda1 - lambda2) ! Covariance matrix elements (1,2) and (2,1)
    a(2,1) = a(1,2)                         ! Symmetric matrix
    a(2,2) = lambda1 * sf2 + lambda2 * cf2  ! Covariance matrix element (2,2)

    ! Use LAPACK dsyev to compute the eigenvalues and eigenvectors
    call dsyev('V', 'U', 2, a, 2, eigenvalues, work, 3, info)
    ...
    ! The matrix `a` now contains the eigenvectors, and `eigenvalues` contains the eigenvalues.
    ! Update the covariance matrix with the values from dsyev (for further calculations if needed)
    t11 = a(1,1)
    t12 = a(1,2)
    t22 = a(2,2)
