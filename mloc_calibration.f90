!> Procedures related to calibration

module mloc_calibration
   ...
   ! Needed only for indirect calibration
   real, allocatable, dimension(:,:) :: covs ! rdbt_test1, cvzero
   real, allocatable, dimension(:,:) :: cvs ! rdbt_test1, rdbt_test2, cvzero
   real, allocatable, dimension(:) :: deprsv ! cal_shift
   real, allocatable, dimension(:) :: dxos ! cvzero
   real, allocatable, dimension(:) :: otrsv ! cal_shift
   real, allocatable, dimension(:) :: rsvx ! cal_shift, rdbt_test1, rdbt_test2
   real, allocatable, dimension(:) :: rsvy ! cal_shift, rdbt_test1, rdbt_test2
   real, allocatable, dimension(:,:,:) :: scv
   real, allocatable, dimension(:,:,:) :: sgcv ! cal_shift, rdbt_test2
   real, allocatable, dimension(:) :: temp ! cvzero
   real, allocatable, dimension(:,:) :: ys ! cvzero
   double precision, allocatable, dimension(:,:) :: ac ! cvzero
   double precision, allocatable, dimension(:) :: qn ! cvzero
   double precision, allocatable, dimension(:,:) :: tempay ! cvzero
   double precision, allocatable, dimension(:,:) :: unn ! cvzero
   double precision, allocatable, dimension(:,:) :: vnn ! cvzero
   double precision, allocatable, dimension(:,:) :: wnn ! cvzero
   double precision, allocatable, dimension(:,:) :: y ! cvzero

   ! Needed for direct calibration
   real, allocatable, dimension(:,:,:) :: dccv ! dircal
   
   ! Needed for both direct and indirect calibration
   real, allocatable, dimension(:,:,:) :: sccv ! cal_shift, dircal
   
contains

!*****************************************************************************************
   subroutine cal_shift (mode, it)
...
! residual vectors is used.
      ...
      integer :: nr
      integer :: nrc
      real, dimension(4,4) :: accv_1
      real, dimension(4,4) :: accv_2
      real, dimension(4,4) :: agcv
      real :: al
      real :: alpha
      real :: area
      real :: avc
...
!      real :: hms2s
      ...
      real :: yl_1
      real :: yl_2
      character(len=12) :: caltype_pr_iev
      ...
      if (mode .eq. 0) then ! Average of individual shift vectors, no weighting
         ...
         write (*,'(/a,i3,a,i3,a/)') 'Indirect calibration: ', ngt5, ' events out of ', n_event, ' are CE05 or better'
         write (io_cal,'(/a,i3,a,i3,a/)') 'Indirect calibration: ', ngt5, ' events out of ', n_event, ' are CE05 or better'
...
!           write (io_cal,'(i3,2f8.2)') iev, rsvx(iev), rsvy(iev)
      ...
      ! Critical value
      if (debug) write (io_log,'(a,i4)') 'rdbt_test1: nr  = ', nr
      if (debug) write (io_log,'(a,i6)') 'rdbt_test1: ntc = ', ntc
      if (debug) write (io_log,'(a,i5)') 'rdbt_test1: nqc = ', nqc
      if (debug) write (io_log,'(a,f10.3)') 'rdbt_test1: shatsqc = ', shatsqc
      mf = (nr - 1)*2
      nf = ntc - nqc - mf
      if (debug) write (io_log,'(a,i4)') 'rdbt_test1: mf = ', mf
      if (debug) write (io_log,'(a,i6)') 'rdbt_test1: nf = ', nf
      call fstat1 (mf, nf, pc, f)
      if (debug) write (io_log,'(a,f10.3)') 'rdbt_test1: f = ', f
...
!      kcrit90 = 1. ! I thought this was correct, since the CVs have already been scaled, but it can't be.
      ...
      if (debug) write (io_log,'(a,f8.1,a)') 'rdbt_test1: critical value = ', kcrit90, ' at 90%'
      ...
      do while (.not.nullhyp)
         ...
         if (rdbt .gt. rvlmax) then
            if (debug) write (io_log,'(a,f4.1,a)') 'rdbt_test1: rdbt_test exceeds rvlmax (', rvlmax,')'
            write (io_cal,'(a,f4.1,a)') 'rdbt_test exceeds rvlmax (', rvlmax,')'
            exit
         end if
      end do
      
      if (debug) write (io_log,'(a,f6.2)') 'rdbt_test1: rdbt1 = ', rdbt
      write (io_cal,'(a,f6.2)') 'rdbt1 = ', rdbt
      
      return
      end subroutine rdbt_test1
...
!*****************************************************************************************
   ...
   ! See Jordan and Sverdrup [1981].
      ...
      real :: avex
      real :: avey
      real, intent(in) :: kcrit90
      real :: kocs2
      real :: sumx
      real :: sumy
      character(len=40) :: nulltext
      character(len=40) :: p = 'cvzero'
      logical, intent(out) :: nullhyp
      
      allocate (indx(n_qi_max), stat=error); if (error .gt. 0) call allocation_error (p, 'indx', error)
      indx = 0
      allocate (irank(n_qi_max), stat=error); if (error .gt. 0) call allocation_error (p, 'irank', error)
      irank = 0
      ...
      avey = sumy/(nr)
      avex = sumx/(nr)
      if (debug) write (io_log,'(a,f10.3)') 'cvzero: avey = ', avey
      if (debug) write (io_log,'(a,f10.3)') 'cvzero: avex = ', avex
      ...
      do i = 1,nr
         ...
         if (debug) write (io_log,'(a,i2,2f10.3)') 'cvzero: dxos ', j, dxos(j*2-1), dxos(j*2)
      end do 
...
!      end do
      ac = dble(covs)
      call dsvd (ac, n, n, n_qi_max, n_qi_max, unn, vnn, qn)
      if (debug) then
         write (io_log,'(a)') 'qn:'
      ...
      if (debug) then
         write (io_log,'(a)') 'indx:'
         write (io_log,'(10i6)') (indx(j),j=1,n)
      end if
      call rank (n, indx, irank)
      if (debug) then
         write (io_log,'(a)') 'irank:'
         write (io_log,'(10i6)') (irank(j),j=1,n)
      end if
      ...
      call ddot3 (wnn, unn, tempay, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n, n, n)
      call ddot1 (vnn, tempay, y, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n_qi_max, n, n, n)
      do j = 1,n
         do i = 1,n
            ys(i,j) = sngl(y(i,j))
         end do
      ...
      call dot1 (ys, dxos, temp, n_qi_max, n_qi_max, n_qi_max, 1, n_qi_max, 1, n, n, 1)
      if (debug) then
         write (io_log,'(a)') 'temp = '
      ...
      deallocate (indx, stat=error); if (error .gt. 0) call allocation_error (p, 'indx', error)
      deallocate (irank, stat=error); if (error .gt. 0) call allocation_error (p, 'irank', error)
...
!************************************************************************
      ...
      if (debug) write (io_log,'(a,2f10.4)') 'c1,c2 = ', c1, c2
...
!            write (io_cal,'(i3,2f8.2)') iev, rsvx(iev), rsvy(iev)
      ...
      if (k .gt. 0) then
         probability = betai(real(k),real(j),pc1)
      else if (k .eq. 0) then
         probability = 1.
      else
      ...
      write (io_cal,'(a,f10.3,3i6,2f10.3,a)') 'rdbt_test = ', rdbt, nr, nrc, k, coverage, probability, trim(nulltext)
...
! the ellipse. covp is less than or equal to 1.0 if the ellipse covers the point.
      ...
      if (debug) write (io_log,'(a,5f10.3)') 'coverage_ellipse: ', dx, dy, cv11, cv12, cv22
      ...
      if (debug) write (io_log,'(a,4f10.3)') 'coverage_ellipse: ', alpha_sminor, sminor, alpha_smajor, smajor
      ...
      if (debug) write (io_log,'(a,4f10.3)') 'coverage_ellipse: ', fi, fir, u, v
      
      ! Substitute point coordinates into ellipse equation
      a = (u*u) / (smajor*smajor)
      b = (v*v) / (sminor*sminor)
      covp = a + b
      if (debug) write (io_log,'(a,3f10.3)') 'coverage_ellipse: ', a, b, covp
      
      return
      end subroutine coverage_ellipse
...
!         write (io_cal,'(i3,3x,a,f10.3)') iev, 'kcrit1 = ', kcrit1
   ...
   !         write (io_cal,'(i3,4f10.3)') iev, dccv(iev,1,1), dccv(iev,1,2), dccv(iev,2,1), dccv(iev,2,2)
      ...
      if (mindx(1,1) .ne. 0 .and. mindx(1,2) .ne. 0) then
         ...
         write (*,'(/a,i3,a,i3,a/)') 'Direct calibration: ', ngt5, ' events out of ', n_event, ' are CE05 or better'
         write (io_cal,'(/a,i3,a,i3,a/)') 'Direct calibration: ', ngt5, ' events out of ', n_event, ' are CE05 or better'
      end if
...
!*****************************************************************************************
   subroutine indirect_calibration_allocate ()
   
   ! Allocate arrays that are used for indirect calibration only in this module. See the
   ! module 'mloc_allocate' for calibration-related variables that are used by several modules.
      ...
      n = n_event
      n2 = n_event*2
      p = 'indirect_calibration_allocate' ! procedure
      
      allocate (ac(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'ac', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': ac allocated (', n2, ',', n2, ')'
      end if
      
      allocate (covs(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'covs', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': covs allocated (', n2, ',', n2, ')'
      end if
      
      allocate (cvs(n,2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'cvs', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': cvs allocated (', n, ',2)'
      end if
      
      allocate (deprsv(n), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'deprsv', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': deprsv allocated (', n, ')'
      end if
      
      allocate (dxos(n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'dxos', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a)') trim(p)//': dxos allocated (', n2, ')'
      end if
      
      allocate (otrsv(n), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'otrsv', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': otrsv allocated (', n, ')'
      end if
      
      allocate (qn(n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'qn', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a)') trim(p)//': qn allocated (', n2, ')'
      end if
      
      allocate (rsvx(n), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'rsvx', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': rsvx allocated (', n, ')'
      end if
      
      allocate (rsvy(n), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'rsvy', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': rsvy allocated (', n, ')'
      end if
      
      ! In some cases, sccv may already be allocated for direct calibration
      if (.not.allocated(sccv)) then
         allocate (sccv(n,4,4), stat=error)
         if (error .gt. 0) then
            call allocation_error (p, 'sccv', error)
         else
            if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': sccv allocated (', n, ',4,4)'
         end if
      end if
      
      allocate (scv(n,4,4), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'scv', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': scv allocated (', n, ',4,4)'
      end if
      
      allocate (sgcv(n,4,4), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'sgcv', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': sgcv allocated (', n, ',4,4)'
      end if
      
      allocate (temp(n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'temp', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a)') trim(p)//': temp allocated (', n2, ')'
      end if
      
      allocate (tempay(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'tempay', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': tempay allocated (', n2, ',', n2, ')'
      end if
      
      allocate (unn(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'unn', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': unn allocated (', n2, ',', n2, ')'
      end if
      
      allocate (vnn(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'vnn', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': vnn allocated (', n2, ',', n2, ')'
      end if
      
      allocate (wnn(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'wnn', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': wnn allocated (', n2, ',', n2, ')'
      end if
      
      allocate (y(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'y', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': y allocated (', n2, ',', n2, ')'
      end if
      
      allocate (ys(n2,n2), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'ys', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i4,a,i4,a)') trim(p)//': ys allocated (', n2, ',', n2, ')'
      end if
      
      return
   
   end subroutine indirect_calibration_allocate
      

!*****************************************************************************************
   subroutine indirect_calibration_deallocate ()
   
   ! Deallocate variable arrays that are used only in this module
   
      p = 'indirect_calibration_deallocate' ! procedure
      
      deallocate (ac, stat=error); if (error .gt. 0) call deallocation_error (p, 'ac', error)
      deallocate (covs, stat=error); if (error .gt. 0) call deallocation_error (p, 'covs', error)
      deallocate (cvs, stat=error); if (error .gt. 0) call deallocation_error (p, 'cvs', error)
      deallocate (deprsv, stat=error); if (error .gt. 0) call deallocation_error (p, 'deprsv', error)
      deallocate (dxos, stat=error); if (error .gt. 0) call deallocation_error (p, 'dxos', error)
      deallocate (otrsv, stat=error); if (error .gt. 0) call deallocation_error (p, 'otrsv', error)
      deallocate (qn, stat=error); if (error .gt. 0) call deallocation_error (p, 'qn', error)
      deallocate (rsvx, stat=error); if (error .gt. 0) call deallocation_error (p, 'rsvx', error)
      deallocate (rsvy, stat=error); if (error .gt. 0) call deallocation_error (p, 'rsvy', error)
      deallocate (sccv, stat=error); if (error .gt. 0) call deallocation_error (p, 'sccv', error)
      deallocate (sgcv, stat=error); if (error .gt. 0) call deallocation_error (p, 'sgcv', error)
      deallocate (temp, stat=error); if (error .gt. 0) call deallocation_error (p, 'temp', error)
      deallocate (tempay, stat=error); if (error .gt. 0) call deallocation_error (p, 'tempay', error)
      deallocate (unn, stat=error); if (error .gt. 0) call deallocation_error (p, 'unn', error)
      deallocate (vnn, stat=error); if (error .gt. 0) call deallocation_error (p, 'vnn', error)
      deallocate (wnn, stat=error); if (error .gt. 0) call deallocation_error (p, 'wnn', error)
      deallocate (y, stat=error); if (error .gt. 0) call deallocation_error (p, 'y', error)
      deallocate (ys, stat=error); if (error .gt. 0) call deallocation_error (p, 'ys', error)
      
      return
   
   end subroutine indirect_calibration_deallocate
      

!*****************************************************************************************
   subroutine direct_calibration_allocate ()
   
   ! Allocate arrays that are used for direct calibration only in this module. See the
   ! module 'mloc_allocate' for calibration-related variables that are used by several modules.
   
      integer :: n
      
      n = n_event
      p = 'direct_calibration_allocate' ! procedure
      
      allocate (dccv(n,4,4), stat=error)
      if (error .gt. 0) then
         call allocation_error (p, 'dccv', error)
      else
         if (verbose_log) write (io_alloc_log,'(a,i3,a)') trim(p)//': dccv allocated (', n, ',4,4)'
      end if
      
      ! In some cases, sccv may already be allocated for indirect calibration
      if (.not.allocated(sccv)) then
         allocate (sccv(n,4,4), stat=error)
         if (error .gt. 0) then
            call allocation_error (p, 'sccv', error)
   ...
   end subroutine direct_calibration_allocate   
      

!*****************************************************************************************
   subroutine direct_calibration_deallocate ()
   
   ! Deallocate variable arrays related to direct calibration
   
      p = 'direct_calibration_deallocate' ! procedure
      
      deallocate (dccv, stat=error); if (error .gt. 0) call deallocation_error (p, 'dccv', error)
      deallocate (sccv, stat=error); if (error .gt. 0) call deallocation_error (p, 'sccv', error)
      
      return
   
   end subroutine direct_calibration_deallocate
      

!*****************************************************************************************
end module mloc_calibration
