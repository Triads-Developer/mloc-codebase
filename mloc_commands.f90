...
! Limited to 20 characters
   ...
   logical, intent(in) :: command_help
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       '~.hdf files. Maximum 20 characters. This over-writes an annotation given in',&
       'the event record of an MNF file. This command can only be issued in a command',&
       'file.'
      return
   end if
   
   if (.not.cmndfil) then
      call warnings ('proc_anno: "anno" can only be called from a command_file')
      return
   end if

   annotation(iev) = trim(params(1:20))
...
! File of stations that are suspected off reporting bogus depth phases
   ...
   if (command_help) then
      write (*,'(13(a/))')&
       ...
       'at a smaller size in the summary plot of relative depth phases (tt8) and will have',&
       'an asterisk next to their entries in the "~.depth_phases" file.',&
      ...
      if (bdp_list) then
         write (*,'(2a/)') 'bdps file = ', bdp_filnam
      else
         write (*,'(a/)') 'bdps file = unspecified'
      end if
      return
   end if
...
! Can be toggled by using the command without an argument, or set explicitly with arguments
! "on" or "off".
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'readings (being used for the hypocentroid) than others. See Jordan & Sverdrup for',&
       'details. The command takes a single argument, "on" or "off". If the argument is',&
       '"on" the correction is made. Correcting the bias increases the variance of the',&
       'hypocentroid location slightly. Compared to the main source of bias, i.e., travel',&
       'time model inadequacies, this is usually negligible, especially in an uncalibrated',&
       'relocation. Default is "on".'
...
! by Ezgi Karasözen 3/9/2016
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       '"bloc" (BayesLOC formatted output file) specifies that an output file (~.bloc)',&
       'will be written. The format is designed for easy import of mloc calibrated',&
      ...
      write (*,'(a,1l/)') 'Current value = ', blocout
      return
   end if
...
! "on" or "off".
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'to the pP time. No calculation is done for swP. Default is "on". The status can',&
       'be toggled on and off by issuing the command without an argument, or set explicitly',&
       'with arguments "on" and "off".'
...
!*****************************************************************************************
subroutine proc_cal (iev, command_help)
...
! Calibration event
   ...
   if (command_help) then
      write (*,'(16(a/))')&
       ...
       'event, i.e., indirect calibration. Epicenters are always assumed to be calibrated.',&
       'The 4th character in the command is used to specify which of the other hypocentral',&
       'parameters are to be considered calibrated:',&
       ...
       'parameters. This command can only be issued from a command file, not interactively.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_cal: "calX" can only be called from a command_file')
      return
   end if
...
!*****************************************************************************************
subroutine proc_cfil (command_file_name, command_help)

! Specify a command file

   character(len=*), intent(in) :: command_file_name
   ...
   if (command_help) then
      write (*,'(11(a/))')&
       '"cfil" (Command FILe) specifies a file of commands that will be read and processed',&
       'before any interactive commands are processed. The "cfil" command is issued internally',&
       'by the mloc code and cannot be issued by the user in any way. Specification of the',&
       'command file is required interactively for each run of mloc. It is minimally',&
       'required to contain the commands that make up the basic event definition blocks:',&
       '  memb',&
       '  even {event name}',&
       '  inpu {event data filename}',&
       'In most cases, however, the command file will also need to contain other important',&
       'commands that are cluster- or event-specific. Some commands can only be issued in',&
       'a command file. Only one command_file can be read for each run.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_cfil: "cfil" cannot be issued by the user, only by mloc itself')
      return
   end if

   filnam = trim(datadir)//dirsym//trim(command_file_name)
   inquire (file=filnam,exist=ex)
   if (ex) then
      call open_file (io_cfil, filnam, 'old')
      if (verbose_screen) call fyi ('proc_cfil: file '//trim(filnam)//' opened as command file')
   else
      call oops ('proc_cfil: '//trim(filnam)//' does not exist')
   end if
...
! Up to three distance ranges can be specified.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'for example, readings at short distances, in the Pdiff range, and near the caustic',&
       'for PKP phases. Since each range requires two values, this command requires',&
       '2, 4, or 6 arguments. It must be said, however, that such caution has never',&
...
! Comment line
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'As alternatives to the "comm" command, the characters "!" and "#" are both recognized',&
       'as causing the following text to be treated as a comment. They can be placed in',&
       'column 1 to comment out an entire command line, or after the command and arguments,',&
       'to document the command in some way.'
      return
   end if
...
! Color palette table for plotting topography in GMT
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'is defined in the initialization routine.'
      write (*,'(2a/)') 'Current value = ', cpt_file
      return
   end if
...
! Calibration type.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
      '"ctyp" (Calibration TYPe) is used to resolve an ambiguity in indirect calibration',&
      'mode, in which it applies only to calibration events. It determines the way in',&
      'which the uncertainty for calibration events is calculated. There are three options,',&
      ...
      write (*,'(2a/)') 'Current value = ', icaltype_pr
      return
   end if
...
! Specification of an additional uncertainty ('fudge factor') in cluster vectors
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       '"cvff"  (Cluster Vector Fudge Factor) specifies a radius (km) for additional',&
       'uncertainty that will be added to the cluster vector covariance matrices to',&
       'account for bias from non-gaussian components of the arrival time data sets.',&
       'This is completely ad hoc, but it goes in the right direction. A value of about',&
       '1.0 km has been inferred from some tests, but further testing is needed.'
      write (*,'(a,f4.1/)') 'Current value = ', radius_cvff
      return
   end if
   ...
   if (nvar .eq. 1) then
      radius_cvff = value(1)
      if (verbose_screen) then
         write (msg,'(a,f4.1)') 'proc_cvff: fudge factor ', radius_cvff
         call fyi (trim(msg))
      end if
   else
...
! Cluster vector travel time error term
   ...
   if (command_help) then
      write (*,'(10(a/))')&
       '"cvtt" (Cluster Vector Travel Time) adds an additional variance to differential',&
       'time data. When using differential arrival times from waveform cross-correlation,',&
       'a pure reading of uncertainty is usually provided, but the actual variance includes',&
       'a component from inadequate theoretical differential travel times. This command',&
       'allows that term to be defined. It is only relevant for differential time data.',&
       'The correction is never added to bulletin data because this error term is already',&
       'absorbed in the empirical reading error. The two parameters are "vmr" (velocity',&
       'model variance), a percent value of velocity variance in the travel time model,',&
       '(default value 0.05, or 5%), and "cscale", a measure of the scale of the cluster',&
       'in km (default 10 km).'
      write (*,'(a,f4.2,1x,f5.1/)') 'Current values = ', vmr, cscale
      return
   end if
...
! DATF output file
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       '"datf" (DATa Final output file) specifies that a ~.datf output will be written.',&
       'This is a single file with all the events written in MNF format, but with phase',&
       'IDs and flags as they are at the end of the run. This will be different from the',&
       '~.dat0 file that is always written because flags and phase IDs can be changed',&
       'during the run. Arguments "on" and "off" can be used to set the logical state',&
       'explicitly. Issuing the command without an argument toggles the logical state.'
      write (*,'(a,1l/)') 'Current state = ', datfout
      return
   end if
...
! Debug mode, extra output in some routines
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       '"dbug" (DeBUG) specifies that extra information about the run will be written to',&
       'the ~.log file. Issuing the command without arguments toggles the current state.',&
       'Arguments "on" and "off" may be used to set the state explicitly. Default is "off".'
      write (*,'(a,1l/)') 'Current state = ', debug
      return
   end if
   ...
   if (verbose_screen) then
      if (debug) then
         call fyi ('proc_dbug: debugging output is turned on')
      else
         call fyi ('proc_dbug: debugging output is turned off')
      end if
   end if
...
! Direct calibration mode
   ...
   if (command_help) then
      write (*,'(13(a/))')&
       ...
       ' with data (i.e., near-distance data) that has minimal travel time error and so',&
       ' the hypocentroid can be taken as relatively bias-free. The confidence ellipse of the',&
       ' hypocentroid is added to those of the cluster vectors to obtain final estimate of',&
       ' uncertainty for each hypocenter. Four new output files are created:',&
       ...
       'cluster vector for each event. This command can only be issued from the command file.',&
       'The state can be toggled by using the command without an argument, or set explicitly',&
       'with arguments "on" or "off".'
      write (*,'(a,1l/)') 'Current state = ', direct_cal
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_memb: "dcal" can only be called from a command_file')
      return
   end if
...
! Plot a digital elevation model in GMT, using ETOPO1 
! <http://www.ngdc.noaa.gov/mgg/global/global.html>.
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       'added to appropriate GMT scripts (notably for the base map plot), using the ETOPO1',&
       '1-minute gridded topography and bathymetry. The state can be toggled by issuing the',&
       'command without an argument, or set explicitly with arguments "on" or "off". Default',&
       'is "off".'
      write (*,'(a,1l/)') 'Current state = ', plot_dem1
      return
   end if
...
! Plot a high-resolution digital elevation model in GMT.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'plots that benefit from higher-resolution topography than the ETOPO1 DEM employed',&
       'as the default (command dem1). It assumes the user has provided a ".grd" file; the',&
       'argument to the command gives the pathname of that file, relative to the MLOC',&
       'working directory. A good website to obtain such files is:',&
       '   <http://topex.ucsd.edu/gmtsar/demgen/>',&
       'If no argument is given, plotting of topography is turned off.'
      if (plot_dem2) then
         write (*,'(2a/)') 'dem2 file = ', trim(dem2_filename)
      else
         write (*,'(a,1l/)') 'Current state = ', plot_dem2
      end if
      return
   end if
...
!*****************************************************************************************
subroutine proc_dep (iev, command_help)
...
! file for future reference.
   ...
   if (command_help) then
      write (*,'(29(a/))')&
       '"dep_" (DEPth) specifies a starting focal depth for the current event. The 4th',&
       'character is used to specify the kind of information used to constrain the depth.',&
       'At least one argument is required, a positive depth value in km, and it can',&
       'optionally take one or two additional arguments to assign an uncertainty to that',&
       'depth.',&
       ...
       '(shallower) uncertainty, in km. Both values should be positive. An argument (or',&
       'arguments) for uncertainty is optional for all forms of the "dep_" command. The',&
       'default uncertainty is 99 km. A reasonable uncertainty for "depc" form can be taken',&
       'from the spread of constrained depths in the focal depth histogram plot.',&
       ...
       '  The "dep_" command is usually issued multiple times, first (before any events are',&
       'defined) to set a cluster default depth (depc) for any events lacking other depth',&
       'constraint, then issued within the event definition blocks of individual events',&
       'to set depth according to some constraint. The "dep_" command overrides the depth',&
       'read from an event file or from an HDF file.' 
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_dep: "dep_" can only be called from a command_file')
      return
   end if

   call decode (params, value, nvar)
   
   if (nvar .eq. 0 .or. nvar .gt. 3) then
   ...
   if (iev .eq. 0) then ! Cluster default depth, applied to any event that does not have depth explicitly set
      if (comd(4:4) .ne. 'c') then
         call warnings ('proc_dep: only the command "depc" can be used to set default depth')
         return
      end if
      ...
      if (nvar .eq. 1) then
         ...
         depth_default_plus = value(2)
         depth_default_minus = value(2)
      else if (nvar .eq. 3) then
         depth_default_plus = value(2)
         depth_default_minus = value(3)
      end if
...
! Differential time data file
   ...
   if (command_help) then
      ...
      if (diffdat) write (*,'(2a/)') 'Differential time data file = ', trim(diffdatfilnam)
      return
   end if
...
! Plot an ellipse.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       '"ellp" (ELLipse Plot) is used to specify the parameters of an ellipse that will be',&
       'plotted in suitable map plots. The command takes five arguments:',&
       '   latitude of the center point',&
       '   longitude of the center point',&
       '   azimuth of the semi-major axis',&
       '   full length of the major axis, in km',&
       '   full length of the minor axis, in km'
      write (*,'(a,i2,a)') 'The command can be issued up to ', n_ellipse_nmax, ' times.'
      if (ellipse_plot) then
         write (*,'(a)') 'Currently defined:'
         do i = 1,n_ellipse
            write (*,'(2f9.4,3f6.2)') (ellipse_data(i,j),j=1,5)
         end do
      end if
      return
   end if
...
! Make a plot of empirical path anomalies for a specific phase.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'distance is used to set the boundaries of the map.'
      write (*,'(a,i2,a)')  'The command can be issued up to ', n_epa_max, ' times'
      if (epa_plot) then
         write (*,'(a)') 'Currently defined:'
         do i = 1,n_epa_plot
            write (*,'(a,1x,f5.1,1x,i1)') epa_plot_phase(n_epa_plot), epa_plot_distance(n_epa_plot),&
             epa_plot_raypath(n_epa_plot)
         end do
      end if
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
      ...
      if (n_epa_plot .le. n_epa_max) then
...
! Event name
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'a form like YYYYMMDD.HHMM.SS (i.e., 16 characters). Maximum 30 characters. The',&
       'command takes one argument. It can only be issued from a command file.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_even: "even" can only be called from a command_file')
      return
   end if
...
!*****************************************************************************************
subroutine proc_evrp (command_help)

! Make a plot of residuals for a specific event.
   
   logical, intent(in) :: command_help
   character(len=132) :: msg
   character(len=80) :: param1
   character(len=8) :: param2, param3, param4
   
   if (command_help) then
      write (*,'(9(a/))')&
       '"evrp" (EVent Residual Plot) controls creation of a plot of residuals for a specific',&
       'event. Unlike most plots all residuals are plotted, whether or not they were',&
       'flagged for any reason, although the two categories are distinguished by',&
       'different colors: flagged readings in red, unflagged readings in gray. P-type and',&
       'S-type phases are distinguished by different symbols: "x" for P, open circles',&
       'for S. The command takes three arguments: event name and minimum and maximum',&
       'epicentral distance (degrees) to plot. If the first argument is "all" instead of',&
       'an event name, all events are plotted with the distance parameters supplied. The',&
       'command can be issued multiple times.'
      return
   end if

   if (params(1:1) .ne. ' ') then
      call decode3 (params, nvar, param1, param2, param3, param4)
      if (nvar .ne. 3) then
         write (msg,'(a,i1)') 'proc_evrp: incorrect number of arguments = ', nvar
         call warnings (trim(msg))
         return
      end if
      if (verbose_screen) then
         call fyi ('proc_evrp: event = '//trim(param1))
         call fyi ('proc_evrp: delta_min   = '//param2)
         call fyi ('proc_evrp: delta_max  = '//param3)
      end if
      
      evrp = .true.
      if (param1 .eq. 'all') then
         evrp_all = .true.
         call decode (param2, value, nvar)
         evrp_delta_min_all = value(1)
         call decode (param3, value, nvar)
         evrp_delta_max_all = value(1)
      else   
         n_evrp = n_evrp + 1
         evrp_event(n_evrp) = trim(param1)
         call decode (param2, value, nvar)
         evrp_delta_min(n_evrp) = value(1)
         call decode (param3, value, nvar)
         evrp_delta_max(n_evrp) = value(1)
      end if
   else
      call warnings ('proc_evrp: "evrp" requires three arguments')
   end if

end subroutine proc_evrp
...
! Use of data flags
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"flag" (data FLAGs) determines if data flags encountered in event files will be',&
       'honored ("on") or ignored ("off"). "off" ignores ALL data flags. The "nofx"',&
       'command is used when only "x" flags are to be ignored. Issuing the command with',&
       'no argument toggles the current state. Default is "on".'
      write (*,'(a,1l/)') 'Cuurent state = ', dflag
      return
   end if
...
! Fault map for GMT scripts.
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       '"fmap" (Fault MAP) specifies a file of digitized faults or other linear features',&
       'to be included on the appropriate map plots. The standard GMT format:',&
       '   "longitude latitude"',&
       'is used, in free format. The symbol ">" in column 1 can be used to separate',&
       ' multiple line segments. The single argument to the command is the pathname',&
       'relative to mloc. The normal place for these files is the directory:',&
       '   /tables/faults',&
       'The style of line used to draw faults is determined by the variable',&
       '"fault_plot_style" which is set in the initialization subroutine.'
      write (*,'(2a)') 'Current value = ', trim(fault_plot_style)
      write (*,'(a,i2,a/)') 'The command can be issued ', n_fault_map_max,' times.'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      if (n_fault_map .lt. n_fault_map_max) then
         inquire (file=trim(params),exist=ex)
         if (ex) then
            fault_map = .true.
...
! Free parameters for cluster vectors
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       '"frec" (FREee Cluster vector parameters) is used to specify which location',&
       'parameters will be free and which will be fixed for cluster vectors. It requires',&
       'four integer arguments that are either 0 or 1. 0 indicates a fixed parameter, 1',&
       'indicates a free parameter. The order is latitude, longitude, depth, origin time.',&
       '"frec" can only be called from a command file. If "frec" is invoked before any',&
       'events are declared, the values apply to all events and this is the normal usage.',&
       'If "frec" is invoked after an event is declared the values only apply to the',&
       'current event. Virtually no testing has been done on this configuration, so',&
       'careful inspection of the output would be warranted.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_frec: "frec" can only be called from a command_file')
      return
   end if
...
! Free parameters for hypocentroid
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"freh" (FREe Hypocentroid parameters) is used to specify which location parameters',&
       'will be free and which will be fixed for the hypocentroid. It requires four',&
       'arguments that are either 0 or 1. 0 indicates a fixed parameter, 1 indicates a',&
       'free parameter. The order is latitiude, longitude, depth, origin time.'
      return
   end if
...
! Global Catalog of Calibrated Earthquake Locations (GCCEL) output file.
   ...
   if (command_help) then
      write (*,'(13(a/))')&
       '"gcat" (Gccel CATalog) causes a folder named ~_gccel to be created and specifies',&
       'that six files will be written to it. One is a ~_gccel.dat file, the primary',&
       'data file for GCCEL. Two files that are intended to be added to existing master KML',&
       'files for display in Google Earth are created, one for the hypocentroid and one for',&
       'the individual epicenters. A ~_plots.pdf file is written, holding the main',&
       'output plots from the run. Copies of the appropriate ~.hdf file and the ~.summary',&
       'are now put into the ~_gccel subdirectory for convenience. These are included in',&
       'the GCCEL distribution at the ISC Dataset Repository to provide additional information',&
       'about the cluster and its calibration. The command takes a single optional argument,',&
       'the filename (assumed to be in the data directory) of a text file containing a',&
       'commentary on the nature of the cluster and the relocation procedures and results.',&
...
! Command summaries and full help
   ...
   write (*,'(t3,a)') 'test: test mode for code development'
   ...
   write (*,'(t3,a)') 'evrp: residual plots for individual events'
...
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(12(a/))')&
       '"ifsm" (Isc-Fdsn Station Metadata) determines if the combined ISC-FDSN station',&
       'metadata (IFSM) file will be searched for missing station codes after the master',&
       'station file and any supplemental station files have been searched. Any station',&
       'codes that are found in the IFSM will NOT be used in the current run; a supplemental',&
...
! Specify an input data file for the current event.
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       'Native Format). The maximum length of the input filename is 20 characters. The command',&
       'can only be issued from a command_file.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_inpu: "inpu" can only be called from a command_file')
      return
   end if
...
! Kill a block of events
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       '"kill" provides a convenient way to kill blocks of events in a command file. It',&
       'requires one argument, either "on" or "off". After the "kill on" command is issued,',&
       'no other commands will be processed until the "kill off" command is processed. If',&
       'the last event in a command file is in a killed block, it must be followed by a',&
       '"kill off" command in order for mloc to run. To kill a single event, the "memb"',&
       'command with the argument "kill" is preferred. The command can only be issued',&
       'from a command_file'
      return
   end if
   
   if (.not.cmndfil) then
      call warnings ('proc_kill: "kill" can only be called from a command_file')
      return
   end if
...
! Starting latitude
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       ...
       'be used as the starting value. This command can only be issued from a command file.'
      return
   end if
   
   if (.not.cmndfil) then
      call warnings ('proc_lat: "lat" can only be called from a command_file')
      return
   end if
...
! Lg travel time calculations.
   ...
   if (command_help) then
      write (*,'(2(a/))')&
       '"lgtt" (LG Travel Time) specifies an intercept (sec), slope (sec/degree) and',&
       ' minimum epicentral distance (degrees) to be used in calculating travel tiems for Lg.'
      write (*,'(a,f5.2,1x,f7.4,1x,f3.1)') 'Current values = ', lg_a, lg_b,lg_min
      return
   end if
...
! Local velocity model
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       '"lmod" (Local MODel) specifies a velocity model to be used to calculate travel',&
       'times at short epicentral distance ranges. Local velocity models that may be used',&
       'for multiple clusters are may be stored in the "tables/crust/" subdirectory. For',&
       'most clusters it is stored in the data directory for the cluster. The format of',&
       'local velocity model files is the format used by HYPOSAT. If a custom velocity',&
       'model is not specified, the global tau-p model is used for all distances. The ',&
       'command takes one argument the pathname (relative to mloc) of the model file. The',&
       'distance and depth ranges for which it will be used are specified in the file itself.'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call open_file (io_locmod, filnam, 'old')
...
! Starting longitude
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'from the input data file will be used as the starting value. "long" can only be',&
       'called from a command_file'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_long: "long" can only be called from a command_file')
      return
   end if
...
! Longitude range
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       '"lonr" (LONngitude Range) specifies the range to which longitudes should be converted.',&
       ...
       'same range.'
      write (*,'(a,i3/)') 'Current value = ', longitude_range
      return
   end if
...
! Threshold for file with large cluster residuals
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       '"lres" (Large RESidual) specifies the threshold value of "eci" used in writing the,',&
       '~.lres output file, containing all readings with excessive cluster residuals (eci).',&
       'The default value is 3.0. It is seldom necessary to change the threshold',&
       'since the "lres" utility program allows the user to set the threshold higher for',&
       'flagging. It is rarely desirable to reduce the threshold much below the default',&
       'value. The ~.lres file is always written for a cluster (i.e., more than one event),',&
       'but not for single event relocations.'
      write (*,'(a,f4.2/)') 'Current value = ', lres
      return
   end if
...
! distance readings.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'are required:',&
       '   1 - the minimum value for local phases (default 0.10)',&
       '   2 - the minimum allowed value for phases beyond local distance (default 0.15)',&
       '   3 - the minimum allowed value for teleseismic depth phases (default 1.0)',&
       'Local phases are defined as those with "g" or "b" as the second character of the',&
       'phase name.'
      write (*,'(a,3f5.2)') 'Current values = ', rderr_min_loc, rderr_min, rderr_min_depth
      return
   end if
...
! Start the specification of a new event
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'argument "kill". Any text following "memb kill" is treated as a comment such as',&
       'the reason for killing the event (e.g., "memb kill azgap"). This is the preferred',&
       'way to kill a single event; see the command "kill" to skip blocks of events. The',&
       '"memb" command can only be issued from a command file.'
      return
   end if
   
   if (.not.cmndfil) then
      call warnings ('proc_memb: "memb" can only be called from a command_file')
      return
   end if
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   
   if (iev .gt. n_event) n_event = iev ! n_event is incremented here.
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   
...
! "on" or "off".
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'be searched for missing station codes before the master station file and any',&
...
! Output file of phase readings over a limited distance range.
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'required.'
      write (*,'(a,2f6.1)') ' Current values = ', dist1, dist2
      return
   end if
...
! P_ travel time calculations.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       ...
       'from which to calculate travel times for P_.'
      write (*,'(a,f5.2,1x,f8.4,1x,f5.2/)') 'Current values = ', p__a, p__b, p__min
      return
   end if
...
! Perturb the starting location of all events
   ...
   if (command_help) then
      ...
      write (*,'(a,2f6.2/)') 'Current values = ', hlatshift, hlonshift, hdepthshift, htimeshift
      return
   end if
...
! May be issued multiple times. Plots are put into a subdirectory.
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'distance to plot.'
      write (*,'(a,i2,a)') 'The command can be issued up to ', n_phrp_max, ' times.'
      return
   end if
...
! It also keeps S-P readings if direct calibration is being used.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       ...
       'hypocentroid. In cases where the Sg arrival time data have major problems this',&
       'command could be used in direct calibration. If direct calibration is being used,',&
       'S-P readings will be retained as well. If no argument is given, the status is',&
       'toggled. The arguments "off" and "on" can be used to set the state explicitly.',&
       'Default = "on".'
      write (*,'(a,1l/)') 'Current state = ', ponly
      return
   end if
...
! plog output file
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'explicitly. Issuing the command without an argument toggles the logical state.',&
       'Default is "off".'
      write (*,'(a,1l/)') 'Current state = ', plogout
      return
   end if
...
! Selected events for plotting
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       ...
       'an integer which specifies a selected-event plot in which this event will be',&
       'included. Multiple selected-event plots can be designated, and an event can',&
       'belong to more than one by issuing the "plot" command several times before',&
       'the following "memb" command. "plot" can only be called from a command file'
      write (*,'(a,i2/)') 'Maximum number of selected event plots = ', n_selected_max
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_plot: "plot" can only be called from a command_file')
      return
   end if
   ...
   if (iev .gt. 0) then
      ...
      if (nvar .eq. 1) then
         iplot = nint(value(1))
         if (iplot .ge. 1 .and. iplot .le. n_selected_max) then
            plot(iev,iplot) = .true.
            if (verbose_screen) then
...
!  character(len=1) :: answer
   ...
   if (command_help) then
      write (*,'(16(a/))')&
       '"pltt" (PLot Travel Time) specifies one or more TT vs distance plots to be made.',&
       'There are nine plot types available, which are specified by the indices 1-9 after',&
       'the command. Up to nine arguments can be given in a single instance of the command:',&
       ...
       'of the hypocentroid.'
      write (*,'(a,9(l1,1x/))') 'Current state = ', tt1, tt2, tt3, tt4, tt5, tt6, tt7, tt8, tt9
...
! multiple times.
   ...
   if (command_help) then
      write (*,'(2(a/))')&
       '"ppri" (Prevent Phase Re-Identification) specifies phases that cannot be renamed.',&
       'It takes one argument, a phase name. The command can be issued multiple times.'
      write (*,'(a,i2/)') 'Maximum number of instances = ', n_no_phreid_max
      return
   end if
...
!*****************************************************************************************
subroutine proc_pslp (command_help)

! Add slip values to the basemap and any selected event plots. 

   character(len=80) :: param1
   character(len=8) :: param2
   character(len=8) :: param3
   character(len=8) :: param4
   logical, intent(in) :: command_help
   logical :: ex

   if (command_help) then
      write (*,'(14(a/))')&
       '"pslp" (Plot SLiP) adds the surface projection of rupture slip values to the',&
       'basemap plot and any selected event plots. The command takes two arguments, a',&
       'pathname to the data file containing slip data, and a color (standard GMT colors)',&
       'for the plot (useful when slip data for more than one event are to be plotted).',&
       'The slip data are represented by a field of transparent circles with sizes',&
       'proportional to slip, and an overlay of contours of the slip at values of 0.2,',&
       '0.4, 0.6, 0.8 and 1.0 meters. The command can be issued multiple times.',&
       'The first (or only) three columns of the data table should be latitude, longitude,',&
       'and slip (m). The grid is assumed to be on 1 km intervals. The first line is',&
       'assumed to be a header line, but if not it just gets skipped. If the data table',&
       'is organized or scaled differently, some of the options in the "awk" command and/or',&
       'the GMT commands "psxy", "xyz2grd and "grdcontour" in the subroutine "base_map" of',&
       'the module mloc_plots.f90 must be modified in the source code or in the GMT script',&
       'that was produced.'
      return
   end if
   ...
   if (nvar .ne. 2) then
      call warnings ('proc_pslp: "pslp" requires two arguments, a path name and color')
   else
      inquire (file=trim(param1),exist=ex)
      if (ex) then
         pslp = .true.
         n_pslp = n_pslp + 1
         pslp_filnam(n_pslp) = trim(param1)
         pslp_color(n_pslp) = trim(param2)
         if (verbose_screen) then
            write (*,'(t3,a,i2,2a)') 'proc_pslp(',n_pslp,'): data file: ', trim(pslp_filnam(n_pslp))
            write (*,'(t3,a,i2,2a)') 'proc_pslp(',n_pslp,'): color: ', trim(pslp_color(n_pslp))
         end if
      else
         call warnings ('proc_pslp: file '//trim(param1)//' does not exist')
      end if
   end if

   return
   
end subroutine proc_pslp
...
! Perfect theoretical travel times for the hypocentroid
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'and its uncertainty. The main use is for comparison of results with location codes',&
       'that do not take travel time variability into account. Phase spread values (either',&
       'default or read from a .ttsprd file) are still used in the windowing algorithm.',&
       'Issuing the command with no argument toggles the current state. Default is "off".'
      write (*,'(a,1l/)') 'Current state = ', pttt
      return
   end if
...
! Read agency and deployment fields from station files and event data files for a specified station.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'up to the limit specified by "n_radf_max".'
      write (*,'(a,i3)') ' n_radf_max = ', n_radf_max
      return
   end if
...
! Do not confuse with command "mare" which only sets a lower limit for empirical reading errors.
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       'for direct-arriving phases Pg and Sg within a specified distance. This is mainly',&
       'used for single event location of events with local data. Three arguments are',&
       'required, the reading error for P and S phases, respectively, and the epicentral',&
       'distance (degrees) to which these values apply.'
      write (*,'(/a,2(f5.2,a),f4.2,a)') 'Current values: ', rderr_loc_p, ' (P) /', rderr_loc_s,&
       '(S) /', rderr_loc_delt, ' deg'
      return
   end if
...
! Review of currently-selected parameters and options
   ...
   write (*,'(t3,a,i3,a,i3,a)') 'Number of events in cluster = ', n_event, '  (', ncal(1), ' calibration events)'
   if (n_event .ge. 1) then
      do jev = 1,n_event
   ...
   if (evrp) then
      do i = 1,n_evrp
         write (*,'(t3,a,t50,a)') '(evrp) residual plot for ', trim(evrp_event(i))
      end do
   end if
   ...
   if (pslp) then
      do i = 1,n_pslp
         write (*,'(t3,a,t50,4a)') '(pslp) slip data plotted for', ': ', pslp_filnam(i), ' in ', pslp_color(i)
      end do   
   end if
   ...
   if (star_plot) then
      if (star_auto) then
         write (*,'(t3,a,t50,a)') '(star) plot stars for events', ': auto'
      else
         do i = 1,n_star_max
            write (*,'(t3,a,t50,a,1x,2a,f3.1)') '(star) plot star for event', ': ', iev_star(i),&
             evtnam(iev_star(i)), 'at', star_size(i)
         end do
      end if
   end if
   if (subc_set) then
...
! Starting locations from an HDF file.
   ...
   if (.not.cmndfil) then
      call warnings ('proc_memb: "rhdf" can only be called from a command_file')
      return
   end if
...
!*****************************************************************************************
subroutine proc_run (command_help)
...
! Begin the relocation
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'relocation process, but no further opportunities to issue any commands.'
      return
   end if
   ...
   if (debug) then
      call fyi ('proc_run: running in test mode!')
      write (io_log,'(a/)') 'Running in test mode'
   end if
...
! Supplementary station file
   ...
   if (command_help) then
      write (*,'(15(a/))')&
       ...
       'called multiple times, up to the value of n_sstn_max.'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      if (n_supp_stn_file .lt. n_sstn_max) then
         inquire (file=trim(params),exist=ex)
         if (ex) then
...
! Plot a star at the location of a specific event.
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'of an event in the various map plots by GMT. This is normally used to highlight',&
       'events of special interest, such as a mainshock and major aftershocks. The command',&
       'has two modes, "manual" and "auto". In manual mode the command is issued within an',&
       'event definition block and takes one argument, the size of the circumscribing circle',&
       '(recommended sizes are in the range 0.1-0.5). In auto mode, the assignment of size',&
       'for each event is done when the event data file is read, based on the magnitude.',&
       'Magnitude-size relations are read from the file "/tables/gmt/star_mag_size.dat".',&
       'Auto mode overrides manual mode. The command can only be issued in the command file.'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_star: "star" can only be called from a command_file')
      return
   end if
   
   if (params(1:4) .eq. 'auto') then ! Auto mode
   
      io_star_auto = lunit ()
      filename = trim(star_auto_path)//dirsym//trim(star_auto_file)
      inquire (file=filename,exist=ex)
      if (ex) then
         star_plot = .true.
         star_auto = .true.
         call open_file (io_star_auto, trim(filename), 'old')
         write (io_log,'(a)') 'proc_star: star plot magnitude table:'
         do i = 1,n_star_table
            read (io_star_auto,'(3(f3.1,1x))') star_table(i,1), star_table(i,2), star_table(i,3)
            write (io_log,'(2x,3f4.1)') (star_table(i,j),j=1,3)
         end do
         close (io_star_auto)
      else
         call warnings ('proc_star: file '//trim(filename)//' does not exist')
      end if
...
! Plot a station location as a triangle.
   ...
   if (command_help) then
      ...
      write (*,'(a,i3,a)') '"stat" can be issued up to', n_stat_max,' times.'
      return
   end if
...
! "step 0" will not run any inversions.
   ...
   if (nvar .eq. 1) then
      if (int(value(1)) .ge. 0 .and. int(value(1)) .le. n_it_max) then
         nstep = int(value(1))
         if (verbose_screen) then
         ...
         write (msg,'(a,i1)') 'proc_step: number of steps must be between 0 and ', n_it_max
         call warnings (trim(msg))
      end if
   else
...
!*****************************************************************************************
subroutine proc_test (command_help)

! Used for testing changes in the code.

   logical :: command_help

   if (command_help) then
      write (*,'(4(a/))')&
       '"test" (TEST mode) sets a flag that allows alternative versions of the code to',&
       'be run for testing. If no argument is given, the status is toggled. The arguments',&
       '"off" and "on" can be used to set the state explicitly. Default = "off". The use of',&
       'test mode is documented in the first line of ~.log and ~.summary files.'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_test: illegal argument')
   end if

   if (debug) then
      if (verbose_screen) call fyi ('proc_test: running in test mode')
   else
      if (verbose_screen) call fyi ('proc_test: test mode off')
   end if
...
!   real :: hms2s
   ...
   if (.not.cmndfil) then
      call warnings ('proc_time: "time" can only be called from a command_file')
      return
   end if
...
! Logging of the travel-time computations
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"tlog" (Travel-time LOG) specifies that details of the travel-time calculations',&
       'will be written to a log file (~.tlog). This is used for debugging the code only!',&
       'The output file can exceed 500 MB for moderately large clusters and the output',&
       'is very obscure unless you are tracking it in the code. Issuing the command',&
       'without arguments toggles the current state. The arguments "on" and "off" may be',&
       'used to set the state explicitly. Default is "off".'
      return
   end if
   ...
   if (log_tt) then
      call fyi ('proc_tlog: travel-time calculations will be logged')
   else
      call fyi ('proc_tlog: travel-time calculations will not be logged')
   end if
...
! Output files for tomography.
   ...
   integer :: np
   real, dimension(12) :: xvar
   logical :: command_help
   character(len=8) :: param(4)
   character(len=132) :: msg
   ...
   if (command_help) then
      write (*,'(19(a/))')&
       ...
       'For type 3 output one or two optional arguments can be given, defining the spacing',&
       'of a grid used to define subclusters for the output. This can be desirable for',&
       'clusters that are large in geographic extent, because the empirical path anomaly',&
       'is normally referenced to a single location, the average of the event hypocenters',&
       'contributing to the station-phase of interest. When subdivided, only the events',&
       'in each grid cell are processed for an estimate of the travel time and the output',&
       'is referenced to the average of those hypocenters. If one additional argument is',&
       'provided the latitude and longitude increments of the grid are equal. If two',&
       'additional arguments are are provided, the first is the increment in latitude the',&
       'second is the increment in longitude. Grids are limited to a number defined by the',&
       'parameter "n_cell_max" (100), but this cannot be checked until the run is made since it',&
       'depends on the geographic extent of the cluster. The user should try to estimate it.',&
       'The "tomo" command can be issued multiple times, up to the limit specified by the',&
       'parameter "n_tomo_max" (10).'
      return
   end if
   
   call decode2 (params, np, param(1), param(2), param(3), param(4))
   
   ! Check for legal number of arguments
   if (np .lt. 2 .or. np .gt. 4) then
      write (msg,'(a,i1)') 'proc_tomo: requires 2-4 arguments (', np, ')'
      call warnings (trim(msg))
      return
   end if
   
   nitomo = nitomo + 1
   if (nitomo .le. n_tomo_max) then
      tomo_phase(nitomo) = param(1)(1:len_trim(param(1)))
      read (param(2)(1:1),'(i1)') itomo(nitomo)
      tomo_sub(nitomo) = .false.
      if (itomo(nitomo) .eq. 3 .and. np .gt. 2) then
         tomo_sub(nitomo) = .true.
         write (msg,'(a,i4)') 'proc_tomo: maximum number of cells = ', n_cell_max
         call fyi (trim(msg))
         if (np .eq. 3) then ! Subcluster with equal size cells in degrees
            call decode (param(3), xvar, nvar)
            cell_lat_inc(nitomo) = xvar(1)
            cell_lon_inc(nitomo) = cell_lat_inc(nitomo)
         else if (np .eq. 4) then ! Subcluster with different latitude and longitude cell size
            call decode (param(3), xvar, nvar)
            cell_lat_inc(nitomo) = xvar(1)
            call decode (param(4), xvar, nvar)
            cell_lon_inc(nitomo) = xvar(1)
         end if
      else if (itomo(nitomo) .ne. 3 .and. np .gt. 2) then
         call warnings ('proc_tomo: type 1 and 2 output cannot be subdivided')
      end if
      
      if (verbose_screen) then
         if (tomo_sub(nitomo)) then
            write (msg,'(2a,1x,i1,1x,2f10.3)') 'proc_tomo: tomography output: ', trim(tomo_phase(nitomo)),&
             itomo(nitomo), cell_lat_inc(nitomo), cell_lon_inc(nitomo)
            call fyi (trim(msg))         
         else
            write (msg,'(2a,1x,i1)') 'proc_tomo: tomography output: ', trim(tomo_phase(nitomo)), itomo(nitomo)
            call fyi (trim(msg))
         end if
      end if
   else
...
! have any arrivals within 4°. The plots are put into a subdirectory of the cluster folder.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       ...
       'The command can be issued multiple times. If the argument "all" is given the plots',&
       'are made for every event with arrival time data within 4°.'
      return
   end if
...
! are put into a subdirectory, as for the relative depth phase plots (type 8).
   ...
   if (command_help) then
      ...
      write (*,'(i4)') 'Maximum number of tt5s plots = ', n_tt5s_max
      return
   end if
...
! Verbose_log mode, extra output to log file.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"vlog" (Verbose LOG) specifies that extra information about the run will be',&
       'written to the ~.log file. The command can only be issued from the command file.',&
       'Issuing the command without arguments toggles the current state. The arguments',&
       '"on" and "off" may be used to set the state explicitly. Default is "off".'
      return
   end if

   if (.not.cmndfil) then
      call warnings ('proc_vlog: "vlog" can only be called from a command_file')
      return
   end if
...
! Verbose_screen mode, extra output to standard output
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       '"vscr" (Verbose SCReen) specifies that extra information about the run',&
       'will be written to the output window. It can be issued interactively after the',&
       'command file is read but for full effect (i.e., processing the command file)',&
       'the "vscr on" command should be placed (anywhere) in the command file.',&
       'Issuing the command without arguments toggles the current state. The arguments',&
       '"on" and "off" may be used to set the state explicitly. Default is "off"'
      return
   end if
...
! 7/8/2018: If present, the last instance of character "!" and any following text are removed before processing
   ...
   integer :: i, j, k, nf, nn, cmd_comment
   character(len=4) :: buf
   ...
   if (linein(1:1) .eq. '!' .or. linein(1:1) .eq. '#') return
   
   cmd_comment = index(linein,'!',.true.)
   if (cmd_comment .eq. 0) then ! No comment appended
      line = linein
   else if (cmd_comment .lt. 6) then
      write (msg,'(3a)') 'parse: poorly-formatted command "', trim(linein), '"'
      call warnings (trim(msg))
      return
   else
      k = cmd_comment - 1
...
!  no subroutines called
   ...
   character(len=8) :: fmt
   real :: xvar(12)
   integer :: i, j, k, l, npt, nin, nvar, nd, nf, nl, iv, nw, nlen, ios
...
!*****************************************************************************************
subroutine decode3 (string_in, nvar, param1, param2, param3, param4)
...
! A maximum of four substrings is supported.
   ...
   n = len_trim(string)
   if (n .eq. 0) then
      call warnings ('decode3: empty parameter string')
      return
   end if
   ...
   do i = 1,n
      if (string(i:i) .eq. ' ') then
         ...
         index_blank(nblank) = i
         if (nblank .gt. 3) then
            call warnings ('decode3: too many separators')
            nvar = 0
            return
         end if
      end if
