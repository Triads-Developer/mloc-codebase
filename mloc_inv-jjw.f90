...
subroutine mlocinv(it)
   ...
   integer :: i, ibin, icond, iev, it, j, k, kbayes, kst, m, mf, mt, mt0, mtc, mth, mttotal, nf, nq, nq_allocated, nt, nt_allocated
   real :: al, alphah1, alphah12, alphah2, az, bl, f, inv_cpu_finish, inv_cpu_start, kcrit, minimum_weight, pc, ph, t11, t12, t22, temp, totimp, totimpiev, xl1h1, xl1h12, xl1h2, xl2h1, xl2h12, xl2h2
   double precision, dimension(4,4) :: awa, v44, vhath1, vhath2
   double precision :: dtemp, e2, ehati, shatsqh, tf
   double precision, dimension(4) :: dx01, dx02, q4, vh
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   
   ...
   if (n_event .gt. 1) then
      ...
      call perform_svd(nt, mt, qbahat, nt_allocated, n_fp_max, unm, vmm, qm, 'A', 'A', 'SVD')
...
!         end do
      ...
      write (*,'(a)') '   ...calling DGESVD...'
      call perform_svd(mt, mt, aqbahat, n_fp_max, n_fp_max, umm, vmm, qm, 'A', 'A', 'SVD')
      call dindexx(mt, qm, indx) 
      call rank(mt, indx, irank)
...
!    write (*,*) size(ahat,1), size(ahat,2)
   ...
   write (*,'(a)') '   ...calling DGESDD...'
   call perform_svd(nq, mth, wa0, nq_allocated, 4, uq4, v44, q4, 'A', 'A', 'SVD')
   call dindexx(mth, q4, indx) 
   icond = int(q4(indx(mth))/q4(indx(1)))
   write (*,'(a,i12)') '   ...return from DGESDD: condition # = ', icond
   do j = 1,mth
      dtemp = 1.0d0/q4(j)
      do i = 1,mth
