!> Tau-P software for global 1-D global travel time model
...
module mloc_taup
   ...
   ! common /umdc/
   integer, dimension(2) :: mt
   integer, dimension(jsrc,2) :: ndex
   double precision, dimension(jsrc,2) :: pm
   double precision, dimension(jsrc,2) :: zm

   ! common /tabc/
   integer, dimension(jbrn,3) :: idel
   integer, dimension(jbrn,2) :: jndx
   integer :: mbr1
   integer :: mbr2
   real :: dn
   real :: hn
   ...
   real :: tn_taup
   real :: xn
   double precision, dimension(jbrn,2) :: dbrn 
   double precision, dimension(jout) :: pt
   double precision, dimension(4,jout) :: tau
   double precision, dimension(2) :: us
   double precision, dimension(jbrn,3) :: xbrn
   double precision, dimension(2,jout) :: xlim

   ! common /brkc/
   integer, dimension(jseg) :: iidx
   integer, dimension(jseg,2) :: indx_taup
   integer, dimension(2) :: int0
   integer, dimension(2) :: isrc
   integer, dimension(jbrn) :: jidx
   integer :: ki
   integer, dimension(jseg) :: kk
   integer, dimension(2) :: km
   integer, dimension(jseg,2) :: kndx
   integer, dimension(2) :: ku
   integer, dimension(2) :: msrc
   integer, dimension(jseg,3) :: nafl
   integer :: nbrn
   integer :: nin
   integer :: nph0
   integer :: nseg
   real, dimension(jseg,3) :: fcs
   real :: odep
   double precision, dimension(5,jout) :: coef
   double precision, dimension(jseg) :: pk
   double precision, dimension(jtsm0,2) :: pu
   double precision, dimension(jxsm,2) :: pux
   double precision, dimension(jbrn,2) :: px
   double precision, dimension(jout) :: taut
   double precision, dimension(jtsm,2) :: tauu
   double precision, dimension(5,jbrna,2) :: tcoef
   double precision, dimension(jbrnu,2) :: tp
   double precision, dimension(jbrn,2) :: xt
   double precision, dimension(jxsm,2) :: xu
   double precision :: zs

   ! common /pcdc/
   character(len=8), dimension(jbrn) :: phcd_taup
   
   ! common /pdec/
   integer, parameter :: ka = 4
   real, parameter :: deplim = 1.1
   double precision, dimension(5,2) :: taua
   double precision, dimension(5,2) :: ua
   
contains

!*****************************************************************************************
subroutine tabin (in, modname)
...
! Read .hed and .tbl files for tau-p calculations.
   ...
   character(len=8), dimension(6) :: phdif
   ...
   if (verbose_screen) call fyi ('Opening '//trim(basename)//'.hed')
   if (log_tt) write (io_tt_log,'(a)') 'tabin: opening '//trim(basename)//'.hed'
   open (nin,file=trim(basename)//'.hed',status='old',form='unformatted')
   rewind (nin)
   
   if (log_tt) write (io_tt_log,'(a,i4)') 'tabin: jseg = ', jseg
   
   read (nin) nasgr, nl, len2, xn, pn, tn_taup, mt, nseg, nbrn, ku, km, fcs, nafl, indx_taup, kndx
   if (log_tt) write (io_tt_log, '(2i6)') nl, len2 ! To avoid 'declared but not referenced' warnings
   read (nin) pm, zm, ndex
   read (nin) pu, pux
   read (nin) phcd_taup, px, xt, jndx
   read (nin) pt, taut
   read (nin) coef
      
   close (nin)
   
   if (verbose_screen) call fyi ('tabin: opening '//trim(basename)//'.tbl')
   if (log_tt) write (io_tt_log,'(a)') 'tabin: opening '//trim(basename)//'.tbl'
...
8     if (jndx(i,2) .le. indx_taup(k,2)) go to 7
      k = k+1
      go to 8
...
9     if (nafl(k,1) .gt. 0 .and. (phcd_taup(i)(1:1) .eq. 'P'.or. phcd_taup(i)(1:1) .eq. 'S')) cycle
      do j = 1,6
         if (phcd_taup(i) .eq. phdif(j)) go to 6
      end do
      cycle
6     dbrn(i,1) = 1d0
      phdif(j) = ' '
...
1  nph0 = 0
   ...
   do i = 1,nseg
      ...
      ind = nafl(i,1)
      k = 0
      do j = indx_taup(i,1),indx_taup(i,2)
         k = k + 1
         pt(j) = tp(k,ind)
      end do
5     iidx(i) = -1
...
7  odep = amax1(dep,.011)
   ...
   hn = 1./(pn*(1.-rdep*xn))
   if (prnt(1) .or. prnt(2)) write (io_tt_log,'(/1x,a,f7.2/)') 'Depth =', dep
...
11    if (jndx(j,1) .ge. indx_taup(i,1)) go to 10
      j = j + 1
      go to 11
...
!      print *,'spfit:  j int =', j, int 
      ...
      if (j .ge. nbrn) cycle
      j = j + 1
!      print *, 'j jidx indx_taup jndx =', j, jidx(j), indx_taup(i,2), jndx(j,2)
      if (jidx(j) .le. indx_taup(i,2) .and. jndx(j,2) .gt. 0) go to 10
   end do

   if (log_tt) then
      write (io_tt_log,*) 'depset: mbr1 mbr2', mbr1, mbr2
      write (io_tt_log,*) 'depset: msrc isrc odep zs us', msrc, isrc, odep, sngl(zs),&
       sngl(us(1)), sngl(us(2))
      write (io_tt_log,'(/10x,i5/(1x,3i5,f12.6))') ki, (i, iidx(i), kk(i), pk(i), i = 1,nseg)
   end if
...
!*****************************************************************************************
      ...
      double precision, dimension(jtsm) :: tauc
      double precision, dimension(2) :: tauus1
      double precision, dimension(2) :: tauus2
      double precision :: ttau
      double precision, dimension(jrec) :: tup
      double precision :: tx
      double precision :: u0
      double precision :: u1
      double precision :: umin
!      double precision :: umod
      double precision, dimension(jxsm) :: xc
      double precision, dimension(2) :: xus1
      double precision, dimension(2) :: xus2
      double precision :: z0
      double precision :: z1
!      double precision :: zmod
      ...
      ! common block needed for equivalencing tup with tauc/xc
      common /taucxc/ tauc, xc
!       data tauc, xc/jtsm*0d0,jxsm*0d0/
      equivalence (tauc,tup) ! common /taucxc/ tauc, xc
      ...
      if (log_tt) write (io_tt_log,*) 'depcor: nph nph0', nph, nph0
      ...
      umin = us(nph)
      ks = isrc(nph)
      if (log_tt) write (io_tt_log,*) 'depcor: ks us', ks, sngl(umin)
      do i = 1,ks
 ...
 4    k2 = i
      if (log_tt) write (io_tt_log,*) 'depcor: k2 umin', k2, sngl(umin)
 ...
 11   do i=1,ku(nph)
      ...
      if (log_tt) write (io_tt_log,*) 'depcor: bkin', ks, sngl(sgn), sngl(tauu(1,nph)),&
       sngl(xu(1,nph))

      ! Fiddle pointers.
 8    msrc(nph) = ks
      if (log_tt) write (io_tt_log,*) 'depcor: msrc sgn', msrc(nph), sngl(sgn)
      noend = .false.
      ...
      k1 = k2 - 1
      if (noend) k1 = k2
      if (log_tt) write (io_tt_log,*) 'depcor: noend noext k2 k1', noend, noext, k2, k1
      if (noext) go to 14
      ...
      mu = 1
      if (log_tt) write (io_tt_log,*) 'depcor: u0 z0', sngl(u0), sngl(z0)
      if (log_tt) write (io_tt_log,*) 'depcor: u1 z1', sngl(u1), sngl(z1)
      do k = 1,k1
         ...
         if (log_tt) write (io_tt_log,*) 'depcor: up x:  k mu', k, mu, sngl(xu(mu,nph)),&
          sngl(xc(mu))
         mu = mu + 1
      end do
      go to 39
 ...
 39   xus1(nph) = 0d0
      ...
      tauus1(nph) = tauc(k1)
      xus1(nph) = xc(mu)
      if (log_tt) write (io_tt_log,*) 'depcor: is ks tauus1 xus1', is, ks,&
 ...
 33   if (pm(is+1,nph) .lt. umin) go to 41
      ...
      if (log_tt) write (io_tt_log,*) 'depcor: is ks tauus2 xus2', is, ks,&
 ...
 41   iph = mod(nph,2) + 1
      ...
      do j = 1,nbrn
         if ((phcd_taup(j)(1:2) .ne. 'sP' .and. phcd_taup(j)(1:2) .ne. 'SP') .or. px(j,2) .le. 0d0) cycle
         if (log_tt) write (io_tt_log,*) 'depcor: j phcd px umin =', j, ' ', phcd_taup(j),&
          px(j,1), px(j,2), umin
 ...
 59   do j = 1,nbrn
         if ((phcd_taup(j)(1:2) .ne. 'pS' .and. phcd_taup(j)(1:2) .ne. 'PS') .or. px(j,2) .le. 0d0) cycle
         if (log_tt) write (io_tt_log,*) 'depcor: j phcd px umin =', j, ' ', phcd_taup(j),&
          px(j,1), px(j,2), umin
         if (umin .ge. px(j,1) .and. umin .lt. px(j,2)) go to 45
      end do
      ...
      ! Do the integral.
 45   j = 1
      if (log_tt) write (io_tt_log,*) 'depcor: do pS or sP integral - iph =', iph
      do i = 2,mt(iph)
 ...
 47   z1 = zmod(umin,j,iph)
      ...
      tauus1(iph) = tauus1(iph) + ttau
      xus1(iph) = xus1(iph) + tx
      if (log_tt) write (io_tt_log,*) 'depcor: is ks tauusp xusp', j, ks,&
...
!     if (odep .ge. deplim .or. odep .le. .1) go to 43
 ...
 58   du = amin1(1e-5+(odep-.4)*2e-5,1e-5)
      if (log_tt) write (io_tt_log,*) 'depcor: Add:  nph is ka odep du us =', nph, is,&
      ...
      do l = ka,1,-1
         ...
         if (log_tt) write (io_tt_log,*) 'depcor: l k ua taua', l, k, sngl(ua(k,nph)),&
          sngl(taua(k,nph))
 ...
 43   j = 1
      if (log_tt) then
         write (io_tt_log,*) 'depcor: mu', mu
         write (io_tt_log,*) 'depcor: killer loop:'
      end if
      do i = 1,nseg
         if (.not.segmsk(i)) cycle
         if (log_tt) write (io_tt_log,*) 'depcor: i iidx nafl nph', i, iidx(i),&
          nafl(i,1), nph
         ...
         i1 = indx_taup(i,1)
         i2 = indx_taup(i,2)
         if (log_tt) write (io_tt_log,*) 'depcor: i1 i2 sgn iph', i1, i2, sngl(sgn), iph
         m = 1
         do k = i1,i2
...
21          tau(1,k) = taut(k) + sgn*tauc(m)
         ...
         k = i2
         if (log_tt) write (io_tt_log,*) 'depcor: k m', k, m
         go to 24
         
22       if (log_tt) write (io_tt_log,*) 'depcor: k m', k, m
         ...
         pt(k) = umin
         fac = fcs(i,1)
         if (log_tt) write (io_tt_log,*) 'depcor: ki fac', ki, sngl(fac)
         tau(1,k) = fac*(tauus1(iph)+tauus2(iph)+tauus1(kph)+tauus2(kph))+sgn*tauus1(nph)
         if (log_tt) write (io_tt_log,*) 'depcor: nph iph kph tauus1 tauus2 tau =',&
          nph, iph, kph, sngl(tauus1(1)), sngl(tauus1(2)), sngl(tauus2(1)), sngl(tauus2(2)),&
          sngl(tau(1,k))
24        m = 1
26       if (jndx(j,1) .ge. indx_taup(i,1)) go to 25
         j = j + 1
         go to 26
...
37       if (log_tt) write (io_tt_log,*) 'depcor: j jndx jidx', j, jndx(j,1), jndx(j,2),&
          jidx(j), ' ', phcd_taup(j)
         do l = 1,2
...
27          xbrn(j,l) = xt(j,l) + sgn*xc(m)
            if (log_tt) write (io_tt_log,*) 'depcor: x up:  j l m  ', j, l, m
            cycle
            
29          xbrn(j,l) = fac*(xus1(iph)+xus2(iph)+xus1(kph)+xus2(kph))+sgn*xus1(nph)
            if (log_tt) then
               write (io_tt_log,*) 'depcor: x up:  j l end', j, l
               write (io_tt_log,*) 'depcor: nph iph kph xus1 xus2 xbrn =', nph, iph, kph,&
...
!*****************************************************************************************
      double precision function zmod (uend, js, nph)
      
      ! Arguments
      double precision, intent(in) :: uend
      ...
      i = js + 1
      zmod = zm(js,nph) + &
       dlog(dmax1((uend-pm(js,nph))*(dexp(zm(i,nph)- zm(js,nph))-1d0)/(pm(i,nph)-pm(js,nph))+1d0,1d-30))
...
!*****************************************************************************************
      ...
      double precision, dimension(nlen), intent(out) :: buf
...
!   tau, and x are all double precision.
      ...
      double precision :: ptk
      double precision :: sqb
      double precision :: sqi
      double precision :: sqj
      double precision :: sqk
...
!*****************************************************************************************
      ...
      integer :: nn
      double precision :: cn
      double precision :: dbrnch
      double precision :: dmn
      double precision :: dmx
      double precision :: dpe
      ...
      double precision :: pe1
      double precision :: pmn
      double precision :: ptol
      double precision :: scpe0
      double precision :: scpe1
      double precision :: shm
      double precision :: spe0
      double precision :: spe1
      double precision :: tau0
      double precision :: tau1
      double precision :: thm
      double precision :: x0
      double precision :: x1
      double precision :: x180
      double precision :: x360
      double precision :: xmin
      character(len=3) :: disc
      logical :: makgrd
      logical :: newgrd
      
      data dbrnch /2.5307274d0/
      data cn /57.295779d0/
      data x180 /3.1415927d0/
      data x360 /6.2831853d0/
      data xmin /3.92403d-3/
      data dtol /1d-6/
      data ptol /2d-6/

       
      if (prnt(1)) write (io_tt_log,102)
      i1 = jndx(jb,1)
      i2 = jndx(jb,2)
      if (log_tt) write (io_tt_log,*) 'spfit: jb i1 i2 pt =', jb, i1, i2, sngl(pt(i1)),&
       sngl(pt(i2))
 ...
 14   newgrd = .false.
      ...
      if (log_tt) write (io_tt_log,*) 'spfit: px newgrd =', sngl(px(jb,2)), newgrd
      if (.not.newgrd) go to 10
      k = mod(int-1,2) + 1
      if (int .ne. int0(k)) makgrd = .true.
      if (log_tt) write (io_tt_log,*) 'spfit: int k int0 makgrd =', int, k, int0(k), makgrd
      if (int .gt. 2) go to 12
!     call query('Enter xmin:',log)
!     read *,xmin
!     xmin=xmin*xn
      xmin = xn*amin1(amax1(2.*odep,2.),25.)
      if (log_tt) write (io_tt_log,*) 'spfit: xmin =', xmin, xmin/xn
      call pdecu (i1, i2, xbrn(jb,1), xbrn(jb,2), xmin, int, i2_)
      i2 = i2_
      jndx(jb,2) = i2
 12   nn = i2 - i1 + 1
      if (makgrd) call tauspl (1, nn, pt(i1), tcoef(1,1,k))
      if (log_tt) write (io_tt_log,301,iostat=ios) jb, k, nn, int, newgrd, makgrd,&
...
!     call appx(i1,i2,xbrn(jb,1),xbrn(jb,2))
      if (log_tt) write (io_tt_log,300) (i, pt(i), (tau(j,i),j=1,3),i=i1,i2)
 300  format (/(1x,i5,4f12.6))
    ...
    4    if (prnt(1)) write (io_tt_log,100,iostat=ios) disc, j, pt(j), (tau(k,j),k=1,4),&
          (cn*xlim(k,j),k=1,2)
    100  format (1x,a,i5,f10.6,1p,4e10.2,0p,2f7.2)
         dmx = dmax1(dmx,xlim(2,j))
         j = i
      end do
      if (log_tt) write (io_tt_log,100,iostat=ios) '   ', j, pt(j)
      xbrn(jb,1) = dmn
      xbrn(jb,2) = dmx
      xbrn(jb,3) = pmn
      idel(jb,1) = 1
      ...
      if (int .gt. 2) go to 1
      phcd_taup(jb) = phcd_taup(jb)(1:1)
      i = jb
      do j = 1,nbrn
         i = mod(i,nbrn) + 1
         if (phcd_taup(i)(1:1) .eq. phcd_taup(jb) .and. phcd_taup(i)(2:2) .ne. 'P' .and. (pe .ge. px(i,1)&
          .and. pe .le. px(i,2))) go to 9
      end do
      go to 1
      
 9    phcd_taup(jb) = phcd_taup(i)
      if (dabs(pt(i2)-pt(jndx(i,1))) .le. dtol) phcd_taup(jb) = phcd_taup(i-1)
 1    if (log_tt) write (io_tt_log,102)
 102  format ()
      if (dbrn(jb,1) .le. 0d0) go to 3
      dbrn(jb,1) = dmx
      dbrn(jb,2) = dbrnch
      if (log_tt) write (io_tt_log,101,iostat=ios) phcd_taup(jb), (jndx(jb,k),k=1,2),&
       (cn*xbrn(jb,k),k=1,2), xbrn(jb,3), (cn*dbrn(jb,k),k=1,2), (idel(jb,k),k=1,3), int,&
       newgrd, makgrd
 101  format (1x,a,2i5,2f8.2,f8.4,2f8.2,4i3,2l2)
      go to 15
      
 3    if (log_tt) write (io_tt_log,103,iostat=ios) phcd_taup(jb), (jndx(jb,k),k=1,2),&
       (cn*xbrn(jb,k),k=1,2), xbrn(jb,3), (idel(jb,k),k=1,3), int, newgrd, makgrd
 103  format (1x,a,2i5,2f8.2,f8.4,16x,4i3,2l2)
 15   if (mxcnt .gt. mncnt .or. mncnt .gt. mxcnt+1) call warning ('Bad interpolation on '//phcd_taup(jb))
...
!*****************************************************************************************
      ...
      if (log_tt) write (io_tt_log,*) 'pdecu: ua =', sngl(ua(1,int))
      if (ua(1,int) .le. 0d0) go to 17
      if (log_tt) write (io_tt_log,*) 'pdecu: fill in new grid'
      k = i1 + 1
      do i =1,ka
         pt(k) = ua(i,int)
         tau(1,k) = taua(i,int)
...
14       if (dabs(x-xs) .le. xmin) go to 15
 ...
 19   len = k
      if (log_tt) write (io_tt_log,300) (i, pt(i), tau(1,i),i=i1,len)
...
!   Rewritten on 23 December 1983 by R. Buland.
      ...
      integer :: m
      integer :: n2
      double precision :: ali
      double precision :: alr
      double precision :: b1h
      double precision :: b3h
      double precision :: bih
      double precision, dimension(5,i2) :: coef
      double precision, dimension(4) :: d
      double precision, dimension(4) :: d1h
      double precision, dimension(4) :: d3h
      double precision, dimension(5) :: del
      double precision, dimension(5) :: deli
      double precision, dimension(4) :: dih
      double precision, dimension(i2) :: pt
      double precision, dimension(5) :: sdel
      double precision :: th0p
      double precision :: th2m
      double precision :: th2p
      double precision :: th3p
...
!   R. Buland.
      ...
      integer :: n
      integer :: n1
      double precision, dimension(2,100) :: a
      double precision :: alr
      double precision, dimension(3) :: ap
      double precision, dimension(100) :: b
      double precision, dimension(5,i2) :: coef
!       double precision :: g1
      double precision :: gn
      double precision, dimension(4,i2) :: tau
      double precision :: x1
      double precision :: xn
...
!*****************************************************************************************
      subroutine trtm (delta, nphase, tt, dtdd, dtdh, dddp, phcd)
      
      integer :: i
      integer, dimension(max_mpl) :: iptr
      integer :: j
      integer :: k
      integer, intent(out) :: nphase
      real, parameter :: atol = .005
      real, dimension(max_mpl), intent(out) :: dtdd
      real, dimension(max_mpl), intent(out) :: dtdh
      real, dimension(max_mpl), intent(out) :: dddp
      real, intent(in) :: delta
      real, dimension(max_mpl,4) :: tmp
      real, dimension(max_mpl), intent(out) :: tt
      double precision, parameter :: cn = .017453292519943296d0
      double precision, parameter :: dtol = 1d-6
      double precision, parameter :: pi = 3.1415926535897932d0
      double precision, parameter :: pi2 = 6.2831853071795865d0
      double precision, dimension(3) :: x
      character(len=8), dimension(max_mpl) :: ctmp
      character(len=8), dimension(max_mpl), intent(out) :: phcd
 ...
 9    if (dabs(x(1)-pi) .gt. dtol) go to 7
      ...
      x(2) = -10d0
 7    do j = mbr1,mbr2
         if (jndx(j,2) .gt. 0) call findtt (j, x, max_mpl, nphase, tmp, tmp(1,2), tmp(1,3), tmp(1,4), ctmp)
      end do
      
      if (nphase-1 .lt. 0) then
         return
      else if (nphase-1 .eq. 0) then
         iptr(1) = 1
      else
         call r4sort (nphase, tmp, iptr)
      end if
      ...
      do i = 1,nphase
         j = iptr(i)
         if (k .le. 0) go to 8
         if (phcd(k) .eq. ctmp(j) .and. abs(tt(k)-tmp(j,1)) .le. atol) cycle
    8    k = k + 1
         tt(k) = tmp(j,1)
         dtdd(k) = tmp(j,2)
         dtdh(k) = tmp(j,3)
...
!*****************************************************************************************
      subroutine findtt (jb, x0, max, n_out, tt_out, dtdd_out, dtdh_out, dddp_out, phnm)
      ...
      integer :: n_out
      integer :: nph
      real, dimension(max) :: dddp_out
      real :: dp0
      real :: dpn
      real :: dsgn
      real, dimension(max) :: dtdd_out
      real, dimension(max) :: dtdh_out
      real :: hsgn
      real, dimension(max) :: tt_out
      double precision :: arg
      double precision :: delp
      double precision, parameter :: deps = 1d-10
      double precision :: dp
      double precision :: dps
      double precision :: p0
      double precision :: p1
      double precision :: ps
      double precision, parameter :: tol = 3d-6
      double precision :: x
      double precision, dimension(3) :: x0
      character(len=*), dimension(max) :: phnm
      character(len=72) :: msg
      ...
      do ij = idel(jb,1),idel(jb,2)
         ...
         do i = is,ie
            ...
            if (n_out .ge. max) go to 13
            n_out = n_out + 1
            ps = pt(ie) - dp
            tt_out(n_out) = tn_taup*sngl(tau(1,j)+dp*(tau(2,j)+dps*tau(4,j))+ps*x)
            dtdd_out(n_out) = sngl(dsgn*ps)
            dtdh_out(n_out) = hsgn*sqrt(abs(sngl(us(nph)*us(nph)-ps*ps)))
            dddp_out(n_out) = dpn*sngl(.75d0*tau(4,j)/dmax1(dabs(dps),deps))
            phnm(n_out) = phcd_taup(jb)
            in = index(phnm(n_out),'ab')
            if (in .le. 0) go to 8
            if (ps .le. xbrn(jb,3)) phnm(n_out)(in:) = 'bc'
            go to 8
       ...
       2    do jj = 1,2
          ...
          7    if (dp .lt. p1-delp .or. dp .gt. p0+delp) cycle
               ...
               n_out = n_out + 1
               ps = pt(ie) - dp
               tt_out(n_out) = tn_taup*sngl(tau(1,j)+dp*(tau(2,j)+dp*tau(3,j)+dps*tau(4,j))+ps*x)
               dtdd_out(n_out) = sngl(dsgn*ps)
               dtdh_out(n_out) = hsgn*sqrt(abs(sngl(us(nph)*us(nph)-ps*ps)))
               dddp_out(n_out) = dpn*sngl(2d0*tau(3,j)+.75d0*tau(4,j)/dmax1(dabs(dps),deps))
               phnm(n_out) = phcd_taup(jb)
               in = index(phnm(n_out),'ab')
               if (in .le. 0) cycle
               if (ps .le. xbrn(jb,3)) phnm(n_out)(in:) = 'bc'
            end do
       9    if (n_out .gt. le) go to 8
            write (msg,101) phcd_taup(jb), x, dp0, dp, p1, p0
    ...
    12   if (x .lt. dbrn(jb,1) .or. x .gt. dbrn(jb,2)) cycle
         ...
         dps = dsqrt(dabs(dp))
         n_out = n_out + 1
         tt_out(n_out) = tn_taup*sngl(tau(1,j)+dp*(tau(2,j)+dp*tau(3,j)+dps*tau(4,j))+pt(j)*x)
         dtdd_out(n_out) = dsgn*sngl(pt(j))
         dtdh_out(n_out) = hsgn*sqrt(abs(sngl(us(nph)*us(nph)-pt(j)*pt(j))))
         dddp_out(n_out) = dpn*sngl(2d0*tau(3,j)+.75d0*tau(4,j)/dmax1(dps,deps))
         ln = index(phcd_taup(jb),' ') - 1
         if (ln .le. 0) ln = len(phcd_taup(jb))
         phnm(n_out) = phcd_taup(jb)(1:ln)//'diff'
      end do
...
!   27 September 1983 by R. Buland.
      ...
      integer :: ib
      integer :: ij
      integer, dimension(10) :: il
      integer, dimension(n) :: iptr
      integer :: it
      integer, dimension(10) :: iu
      integer :: j
      integer :: k
      integer :: kk
      integer :: l
...
!     print *,'mbr1 mbr2',mbr1,mbr2
      do jp = mbr1,mbr2
!        print *,'jp phcd xbrn',jp,'  ',phcd_taup(jp),xbrn(jp,1)
         if ((phcd_taup(jp) .eq. 'Pg' .or. phcd_taup(jp) .eq. 'Pb' .or. phcd_taup(jp) .eq. 'Pn' .or.&
          phcd_taup(jp) .eq. 'P') .and. xbrn(jp,1) .le. 0d0) go to 3
      end do
      jp = 0
      
      ! Find the upgoing S branch.
 3    do js = mbr1,mbr2
   !     print *,'js phcd xbrn',js,'  ',phcd_taup(js),xbrn(js,1)
         if ((phcd_taup(js) .eq. 'Sg' .or. phcd_taup(js) .eq. 'Sb' .or. phcd_taup(js) .eq. 'Sn' .or.&
          phcd_taup(js) .eq. 'S') .and. xbrn(js,1) .le. 0d0) go to 1
      end do
      js=0
...
!     print *,'j pt dp dps x',j,pt(ie),dp,dps,x
      tcor = tn_taup*sngl(tau(1,j)+dp*(tau(2,j)+dp*tau(3,j)+dps*tau(4,j))+ps*x)
      xcor = sngl(cn*x)
!     print *,'iupcor xcor tcor',iupcor,xcor,tcor
      return
...
!   query mode) are also available.
      ...
      integer, parameter :: lcmd = 16
      integer, parameter :: ncmd = 4
      ...
      integer :: kseg
      integer :: l
      integer, dimension(2,ncmd) :: ncmpt
      integer :: nn 
      integer :: no
      integer, dimension(jbrn) :: nsgpt
      logical :: all
      logical :: fnd
      logical, dimension(3) :: prflg
      character(len=8), dimension(ncmd) :: cmdcd
      character(len=8), dimension(lcmd) :: cmdlst
      character(len=*), dimension(nn) :: pcntl
      character(len=8), dimension(jseg) :: phlst
      character(len=8) :: phtmp
      character(len=8), dimension(jbrn) :: segcd
...
!   doesn't require a keyword.
 ...
 1    all = .false.
    ...
    9    j = j + 1
       ...
       5    if (l .ge. 8) cycle
            if (phtmp(l:l+1) .ne. 'ab' .and. phtmp(l:l+1) .ne. 'ac' .and. phtmp(l:l+1) .ne. 'df') then
               cycle
            else
               phtmp(l:) = ' '
!              go to 4
   ...
   !     print *,'j phcd phtmp =',j,' ',phcd_taup(j),' ',phtmp
   ...
   !     if(prflg(3)) print *,'kseg nsgpt segcd =',kseg,nsgpt(kseg),' ', segcd(kseg)
    8    if (jidx(j) .lt. indx_taup(i,2)) go to 9
      end do
      if (all) go to 24
 ...
 24   if (.not.prflg(3)) return
    ...
    18   do j2 = j1,kseg
         ...
         ! Print the result.
    20   j2 = j2 - 1
         if (.not.fnd .and. verbose_screen) print *,'Brnset:  the following phases have '//'been selected -'
         fnd = .true.
         if (verbose_screen) print 102,i,(segcd(j),j=j1,j2)
